...
[`WebGLRenderer.setAnimationLoop`](/api/en/renderers/WebGLRenderer.setAnimationLoop) and passing a function to call for the loop.
...
This brings up an extremely important point. <strong>Units in VR are in meters</strong>.
...
user _in the real world_. That means we need the units used in three.js to match
...
To use WebXR on Android or iPhone you'll need a _VR Headset_
...
These are _the real deal_ haha. 6 degrees of freedom
...
whatever means you want, for example the [`OrbitControls`](/examples/controls/OrbitControls),
...
[click here](../examples/webxr-basic-vr-optional.html).
