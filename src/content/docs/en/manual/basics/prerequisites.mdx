...
via import and via `<script type="module">` tags. They assume you know how to use import maps.
...
function _closes_ over the variable `foo`. Here's [more info](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures).
...
[_this_ article might help explain `this`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this).

## ES5/ES6/ES7 stuff

### `var` is deprecated. Use `const` and/or `let`

There is no reason to use `var` <strong>EVER</strong> and at this point it's considered bad practice
