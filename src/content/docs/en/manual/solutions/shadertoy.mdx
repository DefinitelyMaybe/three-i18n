...
It's important to recognize it's called Shader<strong>TOY</strong> for a reason. In general
...
In the case of Shadertoy the puzzle is, _write a function that for a given pixel
location outputs a color that draws something interesting_. It's a fun challenge
...
[shadertoy.com](https://shadertoy.com) only concerns itself with _fragment
shaders_. A fragment shader's responsibility is, given a pixel location output
...
_normalized_ values often makes things easier and so the majority of shadertoy
...
In shader jargon these global variables are called _uniform_ variables. They are
called _uniform_ because they don't change, they stay uniform from one iteration
of the shader to the next. It's important to note all of them are specific to
shadertoy. They not _official_ GLSL variables. They are variables the makers of
shadertoy made up.
...
[`OrthographicCamera`](/api/en/cameras/OrthographicCamera) with these parameters and a 2 unit plane will fill the
...
Then we pass both the fragment shader and uniforms to a [`ShaderMaterial`](/api/en/materials/ShaderMaterial).
...
<ul>
<li>`uv` goes from 0 to 1. </li>
<li>`cos(uv.xyx)` gives us 3 cosine values as a `vec3`. One for `uv.x`, another for `uv.y` and another for `uv.x` again.</li>
<li>Adding in the time, `cos(iTime+uv.xyx)` makes them animate.</li>
<li>Adding in `vec3(0,2,4)` as in `cos(iTime+uv.xyx+vec3(0,2,4))` offsets the cosine waves</li>
<li>`cos` goes from -1 to 1 so the `0.5 * 0.5 + cos(...)` converts from -1 < - > 1 to 0.0 < - > 1.0</li>
...
[_procedural texture_](https://www.google.com/search?q=procedural+texture).
...
To do that we add in a _varying_. A varying is a value passed from the vertex
...
texture coordinates as `uv` with the `v` in front meaning _varying_.
...
We need to pass the vertex shader to the [`ShaderMaterial`](/api/en/materials/ShaderMaterial)
