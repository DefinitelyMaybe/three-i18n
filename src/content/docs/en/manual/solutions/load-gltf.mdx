...
We also need to include the [`GLTFLoader`](/examples/loaders/GLTFLoader) and we can get rid of the [`OBJLoader`](/examples/loaders/OBJLoader).
...
another [`Object3D`](/api/en/core/Object3D). We will then move those [`Object3D`](/api/en/core/Object3D) objects
to move the cars but separately we can set the car's original
[`Object3D`](/api/en/core/Object3D) to re-orient it so it's about where we really need it.
...
[`Object3D`](/api/en/core/Object3D), parent that new [`Object3D`](/api/en/core/Object3D) to the scene, and apply
some per car _type_ settings to fix its orientation, and add
the new [`Object3D`](/api/en/core/Object3D) a `cars` array.
...
THREE.js has some curve classes. The [`CatmullRomCurve3`](/api/en/extras/curves/CatmullRomCurve3) seemed
...
and pass the result to [`CatmullRomCurve3`](/api/en/extras/curves/CatmullRomCurve3).
...
To do this I grabbed all the GUI code from the [`DirectionalLight`](/api/en/lights/DirectionalLight) shadows
