...
[`transparent`](/api/en/materials/Material#transparent) flag and to set an
[`opacity`](/api/en/materials/Material#opacity) level with 1 being completely opaque
...
We learned about the [`side`](/api/en/materials/Material#side) material property in
...
drawing the stuff in front. THREE.js does this for objects like [`Mesh`](/api/en/objects/Mesh) otherwise
...
Any with that it _seems_ to work.
...
texture coordinates. Here though we're using [`PlaneGeometry`](/api/en/geometries/PlaneGeometry) which by
default stretches the texture across the plane. Like we [covered
before](textures.html) By setting the [`texture.repeat`](/api/en/textures/Texture#repeat)
and [`texture.offset`](/api/en/textures/Texture#offset) we can scale and move the texture to get
the correct half of the face texture on each plane.

The code above also makes a [`Object3D`](/api/en/core/Object3D) and parents the 2 planes to it.
It seemed easier to rotate a parent [`Object3D`](/api/en/core/Object3D) than to do the math
...
An alpha test is a level of _alpha_ below which three.js will not
...
use these textures and set an [`alphaTest`](/api/en/materials/Material#alphaTest).
