...
<ul>
  ...
  <li>
    **Vertex colors:** `BufferAttribute` store RGB components in the Linear-sRGB
    working color space.
  </li>
  <li>
    **Color textures:** PNG or JPEG `Texture` containing color information (like
    .map or .emissiveMap) use the closed domain sRGB color space, and must be
    annotated with _texture.colorSpace = SRGBColorSpace_. Formats like OpenEXR
    (sometimes used for .envMap or .lightMap) use the Linear-sRGB color space
    indicated with _texture.colorSpace = LinearSRGBColorSpace_, and may contain
    values in the open domain [0,∞].
  </li>
  <li>
