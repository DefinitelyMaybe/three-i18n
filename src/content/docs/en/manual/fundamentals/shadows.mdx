...
Three.js by default uses _shadow maps_. The way a shadow map works
is, _for every light that casts shadows all objects marked to cast
shadows are rendered from the point of view of the light_. <strong>READ THAT
AGAIN!</strong> and let it sink in.
...
a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) as we don't need lighting for the ground.
...
[`THREE.Object3D`](/api/en/core/Object3D) and we'll make both the shadow plane mesh and the sphere mesh
...
The shadow is a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) because it doesn't need lighting.
...
We setup 2 lights. One is a [`HemisphereLight`](/api/en/lights/HemisphereLight) with the intensity set to 2 to really
brighten things up.
...
The other is a [`DirectionalLight`](/api/en/lights/DirectionalLight) so the spheres get some definition
...
move the sphere up and down using [`Math.abs(Math.sin(time))`](</api/en/math/Math.abs(Math.sin(time))>)
...
So, moving on to shadow maps, there are 3 lights which can cast shadows. The [`DirectionalLight`](/api/en/lights/DirectionalLight),
the [`PointLight`](/api/en/lights/PointLight), and the [`SpotLight`](/api/en/lights/SpotLight).

Let's start with the [`DirectionalLight`](/api/en/lights/DirectionalLight) with the helper example from [the lights article](lights.html).
...
of view of the light. In this case there is a camera at the [`DirectionalLight`](/api/en/lights/DirectionalLight)
...
a [`CameraHelper`](/api/en/helpers/CameraHelper) to the scene.
...
[`DirectionalLight`](/api/en/lights/DirectionalLight) represents light all going in a parallel direction, the
[`DirectionalLight`](/api/en/lights/DirectionalLight) uses an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) for its shadow camera.
We went over how an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) works in [the previous article about cameras](cameras.html).

Recall an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) defines
its box or _view frustum_ by its `left`, `right`, `top`, `bottom`, `near`, `far`,
and `zoom` properties.
...
allowed which is available on the renderer as [`renderer.capabilities.maxTextureSize`](/api/en/renderers/WebGLRenderer#capabilities).

Switching to the [`SpotLight`](/api/en/lights/SpotLight) the light's shadow camera becomes a [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera). Unlike the [`DirectionalLight`](/api/en/lights/DirectionalLight)'s shadow camera
where we could manually set most its settings, [`SpotLight`](/api/en/lights/SpotLight)'s shadow camera is controlled by the [`SpotLight`](/api/en/lights/SpotLight) itself. The `fov` for the shadow
camera is directly connected to the [`SpotLight`](/api/en/lights/SpotLight)'s `angle` setting.
...
And finally there's shadows with a [`PointLight`](/api/en/lights/PointLight). Since a [`PointLight`](/api/en/lights/PointLight)
shines in all directions the only relevant settings are `near` and `far`.
Otherwise the [`PointLight`](/api/en/lights/PointLight) shadow is effectively 6 [`SpotLight`](/api/en/lights/SpotLight) shadows
each one pointing to the face of a cube around the light. This means
[`PointLight`](/api/en/lights/PointLight) shadows are much slower since the entire scene must be
...
And of course we need to switch the light to a [`PointLight`](/api/en/lights/PointLight).
