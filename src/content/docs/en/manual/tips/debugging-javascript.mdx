...
Inside all devtools is a _console_. It shows warnings and error messages.

<strong> READ THE MESSAGES!! </strong>
...
If you see any others <strong>READ THEM</strong>. For example:
...
And let's make simple class to manage this _clear back buffer_.
...
Here's the code that adds a new [`Mesh`](/api/en/objects/Mesh) every time we click the mouse
...
a curve using the [`SplineCurve`](/api/en/extras/curves/SplineCurve) class which makes a 2D curve.
...
second argument is `car.position` which is a [`Vector3`](/api/en/math/Vector3). [`Vector3`](/api/en/math/Vector3)'s
`set` function requires 3 arguments, x, y, and z but [`SplineCurve`](/api/en/extras/curves/SplineCurve) is a 2D curve
...
## Making a _Minimal, Complete, Verifiable, Example_ for Stack Overflow
...
The <strong>Minimal</strong> part is important. Let's say you where having an issue with the
...
shadows by using a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial). You can certainly remove the lil-gui
code. The code makes a ground plane with a texture. It would be easier to use a
[`GridHelper`](/api/en/helpers/GridHelper). Finally if our question is about moving things on a path we could
...
Also important, when you go to Stack Overflow to post your question <strong>put your
code [in a snippet](https://stackoverflow.blog/2014/09/16/introducing-runnable-javascript-css-and-html-code-snippets/).</strong>
...
## Use a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial)

Because the [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) uses no lights this is one way to
remove reasons something might not be showing up. If your objects
show up using [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) but not with whatever materials
...
A [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera) has `near` and `far` settings which are covered in the
...
like the [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) and make sure you can get that on the screen.
