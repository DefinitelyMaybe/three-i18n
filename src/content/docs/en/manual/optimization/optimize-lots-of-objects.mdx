...
to as _merging geometry_. Every [`Mesh`](/api/en/objects/Mesh) you create and three.js represents 1 or
...
Of course we could also solve that by parenting the box to more [`THREE.Object3D`](/api/en/core/Object3D)
...
For each data point we create a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) and a [`Mesh`](/api/en/objects/Mesh) and then we ask
for the world matrix of the `positionHelper` and apply that to the new [`Mesh`](/api/en/objects/Mesh).
