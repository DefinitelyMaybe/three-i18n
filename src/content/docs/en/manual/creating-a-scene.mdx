...
If you haven't yet, go through the `Installation` guide. We'll assume you've already set up the same project structure (including _index.html_ and _main.js_), have installed three.js, and are either running a build tool, or using a local server with a CDN and import maps.
