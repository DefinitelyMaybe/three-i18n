...
这是Three.js系列文章的第一篇。
[Three.js](https://threejs.org)是一个尽可能简化在网页端获取3D
内容的库。
...
<ul>
<li>
首先有一个[渲染器(`Renderer`)](/api/zh/constants/Renderer)。这可以说是three.js的主要对象。你传入一个[场景(`Scene`)](/api/zh/scenes/Scene)和一个[摄像机(`Camera`)](/api/zh/cameras/Camera)到[渲染器(`Renderer`)](/api/zh/constants/Renderer)中，然后它会将摄像机视椎体中的三维场景渲染成一个二维图片显示在画布上。

</li>
<li>
其次有一个[场景图](scenegraph.html) 它是一个树状结构，由很多对象组成，比如图中包含了一个[场景(`Scene`)](/api/zh/scenes/Scene)对象 ，多个[网格(`Mesh`)](/api/zh/objects/Mesh)对象，[光源(`Light`)](/api/zh/lights/Light)对象，[群组(`Group`)](/api/zh/objects/Group)，[三维物体(`Object3D`)](/api/zh/core/Object3D)，和[摄像机(`Camera`)](/api/zh/cameras/Camera)对象。一个[场景(`Scene`)](/api/zh/scenes/Scene)对象定义了场景图最基本的要素，并包了含背景色和雾等属性。这些对象通过一个层级关系明确的树状结构来展示出各自的位置和方向。子对象的位置和方向总是相对于父对象而言的。比如说汽车的轮子是汽车的子对象，这样移动和定位汽车时就会自动移动轮子。你可以在[场景图](scenegraph.html)的这篇文章中了解更多内容。

注意图中[摄像机(`Camera`)](/api/zh/cameras/Camera)是一半在场景图中，一半在场景图外的。这表示在three.js中，[摄像机(`Camera`)](/api/zh/cameras/Camera)和其他对象不同的是，它不一定要在场景图中才能起作用。相同的是，[摄像机(`Camera`)](/api/zh/cameras/Camera)作为其他对象的子对象，同样会继承它父对象的位置和朝向。在[场景图](scenegraph.html)这篇文章的结尾部分有放置多个[摄像机(`Camera`)](/api/zh/cameras/Camera)在一个场景中的例子。

</li>
<li>
[网格(`Mesh`)](/api/zh/objects/Mesh)对象可以理解为用一种特定的[材质(`Material`)](/api/zh/materials/Material)来绘制的一个特定的[几何体(`Geometry`)](Geometry)。[材质(`Material`)](/api/zh/materials/Material)和[几何体(`Geometry`)](Geometry)可以被多个[网格(`Mesh`)](/api/zh/objects/Mesh)对象使用。比如在不同的位置画两个蓝色立方体，我们会需要两个[网格(`Mesh`)](/api/zh/objects/Mesh)对象来代表每一个立方体的位置和方向。但只需一个[几何体(`Geometry`)](Geometry)来存放立方体的顶点数据，和一种[材质(`Material`)](/api/zh/materials/Material)来定义立方体的颜色为蓝色就可以了。两个[网格(`Mesh`)](/api/zh/objects/Mesh)对象都引用了相同的[几何体(`Geometry`)](Geometry)和[材质(`Material`)](/api/zh/materials/Material)。
...
<li>
[材质(`Material`)](/api/zh/materials/Material)对象代表[绘制几何体的表面属性](materials.html)，包括使用的颜色，和光亮程度。一个[材质(`Material`)](/api/zh/materials/Material)可以引用一个或多个[纹理(`Texture`)](/api/zh/textures/Texture)，这些纹理可以用来，打个比方，将图像包裹到几何体的表面。

</li>
<li>
[纹理(`Texture`)](/api/zh/textures/Texture)对象通常表示一幅要么[从文件中加载](textures.html)，要么[在画布上生成](canvas-textures.html)，要么[由另一个场景渲染出](rendertargets.html)的图像。

</li>
<li>
[光源(`Light`)](/api/zh/lights/Light)对象代表[不同种类的光](lights.html)。
...
拿到canvas后我们需要创建一个[WebGL渲染器(`WebGLRenderer`)](/api/zh/renderers/WebGLRenderer)。渲染器负责将你提供的所有数据渲染绘制到canvas上。
...
接下来我们需要一个[透视摄像机(`PerspectiveCamera`)](/api/zh/cameras/PerspectiveCamera)。
...
这四个参数定义了一个 _"视椎(frustum)"_。 *视椎(frustum)*是指一个像被削去顶部的金字塔形状。换句话说，可以把"视椎(frustum)"想象成其他三维形状如球体、立方体、棱柱体、截椎体。
...
然后我们创建一个[场景(`Scene`)](/api/zh/scenes/Scene)。[场景(`Scene`)](/api/zh/scenes/Scene)是three.js的基本的组成部分。需要three.js绘制的东西都需要加入到scene中。 我们将会在[场景是如何工作的](scenegraph.html)一文中详细讨论。

```js
const scene = new THREE.Scene();
```

然后创建一个包含盒子信息的[立方几何体(`BoxGeometry`)](/api/zh/geometries/BoxGeometry)。几乎所有希望在three.js中显示的物体都需要一个包含了组成三维物体的顶点信息的几何体。
...
再创建一个[网格(`Mesh`)](/api/zh/objects/Mesh)对象，它包含了：

<ol>
  <li>[几何体(`Geometry`)](Geometry)(物体的形状)</li>
  <li>
    [材质(`Material`)](/api/zh/materials/Material)(如何绘制物体，光滑还是平整，什么颜色，什么贴图等等)
  </li>
  <li>
...
我们还需要改变下立方体的材质。[`MeshBasicMaterial`](/api/zh/materials/MeshBasicMaterial)材质不会受到灯光的影响。我们将他改成会受灯光影响的[`MeshPhongMaterial`](/api/zh/materials/MeshPhongMaterial)材质。
...
正如你看见的那样，我们有三个[网格(`Mesh`)](/api/zh/objects/Mesh)引用了相同的[立方几何体(`BoxGeometry`)](/api/zh/geometries/BoxGeometry)。每个[网格(`Mesh`)](/api/zh/objects/Mesh)引用了一个单独的[`MeshPhongMaterial`](/api/zh/materials/MeshPhongMaterial)材质来显示不同的颜色。
...
之所以需要这种文件夹结构，是因为像[`OrbitControls.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/controls/OrbitControls.js)这样的示例中的脚本有一个复杂的相对路径，像下面这样
...
在使用CDN时，是同样的道理。确保`three.modules.js`的路径以
`/build/three.modules.js`结尾，比如
