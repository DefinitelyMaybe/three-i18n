...
基于[光线文章](lights.html)中的定向光线（[`DirectionalLight`](/api/zh/lights/DirectionalLight)）示例，结合半球光线（[`HemisphereLight`](/api/zh/lights/HemisphereLight)）示例。相对于示例，我删除了所有与调整灯光相关的GUI内容，还删除了添加到场景中的立方体和球体。

第一件要做的事就是将[`OBJLoader`](/examples/loaders/OBJLoader)添加到代码中。

```js
import { OBJLoader } from "three/addons/loaders/OBJLoader.js";
```

然后创建[`OBJLoader`](/examples/loaders/OBJLoader)的实例并通过URL加载我们的.OBJ文件，并在回调函数中将已加载完的模型添加到场景（scene）里。
...
我们所下载的.blend文件是包含了模型的纹理，所以我们可以让blender点击<strong>File->External Data->Unpack All Into Files</strong>导出这些文件。

<div class="threejs_center">
  ![image](/resources/images/windmill-export-textures.jpg)
</div>

选择 <strong>Write Files to Current Directory</strong>

<div class="threejs_center">
  ![image](/resources/images/windmill-overwrite.jpg)
</div>

这样就会在.blend文件的目录下创建出一个<strong>textures</strong>文件夹。
...
首先要引用 [`MTLLoader`](/examples/loaders/MTLLoader);
...
然后我们先加载.MTL文件，在它加载完材质后利用`MtlObjBridge`将材质传给[`OBJLoader`](/examples/loaders/OBJLoader)，再加载.OBJ文件。
...
你可以看到在上面，我们传递了`boxSize * 1.2`，当试图把它放入截锥体时，给了20%额外空间在盒子的上下面。我们还更新了[`OrbitControls`](/examples/controls/OrbitControls)，使相机转动时是围绕它进行转动。
...
THREE.js实际上有一个TGA加载器，但是在大多数情况下使用它是错误的。如果你制作一个查看器，你想让用户查看他们在网上找到的随机3D文件，那么可能，只是可能，你可能想加载TGA文件。([\*](/zh/manual/load-obj#loading-scenes))
...
现在。mtl文件指向一些合理大小的纹理，我们需要加载它，所以我们就像上面做的那样，首先加载材质，然后将它们设置在[`OBJLoader`](/examples/loaders/OBJLoader)上。
...
问题1：三个[`MTLLoader`](/examples/loaders/MTLLoader)创建的材质会将材质的漫反射颜色，乘以漫反射纹理贴图。
