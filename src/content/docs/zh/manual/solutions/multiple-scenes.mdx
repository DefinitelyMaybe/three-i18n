...
<blockquote>
<ol>
<li>浏览器限制了WebGL上下文`(WebGL contexts)`的数量。</li>
</ol>
...
解决办法就是用一张`Canvas`在整个背景中填充视口，并利用一些其他元素来代表每个“虚拟画布”`(virtual canvas)`，即只在一张`Canvas`中加载一个[`Renderer`](/api/zh/constants/Renderer)，并为每个`virtual canvas`创建一个场景`(Scene)`。这样我们只需要确保每个`virtual canvas`正确的位置，THREE.js就会将它们渲染在屏幕上相应的位置。

利用这个方法，由于我们只添加了一张`Canvas`，也就仅仅使用了一个`WebGL contexts`，因此不仅解决了资源共享问题，且不会引发WebGL上下文数量限制问题。

以一个只有两个[`Scene`](/api/zh/scenes/Scene)的简单demo为例。首先，创建HTML结构：

```html
<canvas id="c"></canvas>
<p>
  <span id="box" class="diagram left"></span>
  I love boxes. Presents come in boxes. When I find a new box I'm always excited
  to find out what's inside.
</p>
<p>
  <span id="pyramid" class="diagram right"></span>
  When I was a kid I dreamed of going on an expedition inside a pyramid and
  finding a undiscovered tomb full of mummies and treasure.
</p>
```
...
现在，创建两个[`Scene`](/api/zh/scenes/Scene)，其中一个添加了立方体，另一个为菱形，并分别为这两个[`Scene`](/api/zh/scenes/Scene)添加灯光`（Light）`和相机`（Camera）`。
...
接着创建一个视图信息获取函数`renderSceneInfo()`和视图渲染函数`render()`，用来渲染那些`virtual canvas`所在的元素出现在了可视区域的[`Scene`](/api/zh/scenes/Scene)。只需调用THREE.js的剪裁区域检测[`Renderer.setScissorTest`](/api/zh/constants/Renderer.setScissorTest)方法，THREE.js就能实现仅渲染部分画布内容的功能，同时，我们需要调用[`Renderer.setViewport`](/api/zh/constants/Renderer.setViewport)和[`Renderer.setScissor`](/api/zh/constants/Renderer.setScissor)来分别设定视口大小和剪裁区域。
...
<iframe src="/editor?url=/manual/examples/multiple-scenes-v1.html"></iframe>
...
为了更直观的观察这个现象，我们给每个[`Scene`](/api/zh/scenes/Scene)加上边框，并设置背景颜色：
...
此时，我们[快速滚动屏幕](../examples/multiple-scenes-v2.html)，就会发现这个问题。屏幕滚动时的动画放慢十倍后的效果如下：

<div class="threejs_center">
  ![image](/resources/images/multi-view-skew.gif)
</div>
...
然后，我们将设置`Canvas`的`transform`来移动它，使画布的顶部位于页面当前滚动到的任何部分的顶部。
...
`position: fixed` 会完全禁用画布的滚动，无论其他元素是否已经滚动经过它；
...
从中可以看出，这个函数将遍历每一个包含了所有[`Scene`](/api/zh/scenes/Scene)元素的数组对象，且每个元素都由各自的`elem`和`fn`属性。

这个函数将检查每个[`Scene`](/api/zh/scenes/Scene)元素是否进入可视区域，一旦进入就会调用它的场景初始化函数，并传给它当前的时间和对应的尺寸位置信息。

现在，把每个[`Scene`](/api/zh/scenes/Scene)的信息添加到数组列表中：
...
<iframe src="/editor?url=/manual/examples/multiple-scenes-generic.html"></iframe>
...
<iframe src="/editor?url=/manual/examples/multiple-scenes-controls.html"></iframe>

上面提到的方法在本网站上可以找到很多实例，比如[Three.js 图元](primitives.html)和[Three.js 材质](materials.html) 这两篇文章。
...
<iframe src="/editor?url=/manual/examples/multiple-scenes-copy-canvas.html"></iframe>
