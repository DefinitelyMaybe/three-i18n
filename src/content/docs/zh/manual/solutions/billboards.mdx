---
title: 广告牌(Billboards)
sidebar:
  order: 11
---

在 [上一篇文章](canvas-textures.html) 我们使用了一个 [`CanvasTexture`](/api/en/textures/CanvasTexture)
在人物上创作标签(Labels)和徽标(Badges)。有时我们想制作一些总是面对相机的东西。Three.js提供了 [`Sprite`](/api/en/objects/Sprite) 和
[`SpriteMaterial`](/api/en/materials/SpriteMaterial) 来实现这个功能。

我们修改这个徽标的例子 [使用Canvas作为纹理](canvas-textures.html)，
应用 [`Sprite`](/api/en/objects/Sprite) and
...
然后我们创建一个函数，对每一棵树的树干和树顶创建一个 [`Mesh`](/api/en/objects/Mesh)
，并把它们都加入到一个 [`Object3D`](/api/en/core/Object3D)对象下。
...
这里有11x11或者121棵树，每棵树由12个多边形组成的锥体 +
...
现在写一些代码把对象绘制到纹理中，使用一个 `RenderTarget`，我们提到过使用
`RenderTarget`
来渲染，具体在这篇 [渲染目标](rendertargets.html) 文章里。
...
我们计算一个包含树的盒模型，这和 [加载.obj的文件](load-obj.html) 中提到的方式一致，有一点微小的改变。


我们再次调用 `frameArea`，稍微改写了一下[加载.obj的文件](load-obj.html)。
