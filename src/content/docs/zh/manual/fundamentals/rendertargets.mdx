...
渲染到渲染目标基本上跟通常的渲染一样。首先我们创建一个 [`WebGLRenderTarget`](/api/zh/renderers/WebGLRenderTarget)。
...
然后我们需要一个 [`Camera`](/api/zh/cameras/Camera) 和一个 [`Scene`](/api/zh/scenes/Scene)
...
在上个例子中的 [`Scene`](/api/zh/scenes/Scene) 和 [`Camera`](/api/zh/cameras/Camera) 保持不变，我们将在画布中继续使用它们，只需要添加渲染的物体。
...
关于 [`WebGLRenderTarget`](/api/zh/renderers/WebGLRenderTarget) 的笔记。

<ul>
<li>
默认情况下 [`WebGLRenderTarget`](/api/zh/renderers/WebGLRenderTarget) 会创建两个纹理。 颜色纹理和深度/模版纹理。如果你不需要深度或者模版纹理，你可以通过可选设置取消创建。例如：
