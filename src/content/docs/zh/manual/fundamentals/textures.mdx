...
<ul>
  <li>[你好，纹理](/zh/manual/textures#hello)</li>
  <li>
    [6种纹理，在立方体的每个面上都有不同的纹理。](/zh/manual/textures#six)
  </li>
  <li>[加载纹理](/zh/manual/textures#loading)</li>
  <ul>
    <li>[简单的方法](/zh/manual/textures#easy)</li>
    <li>[等待一个纹理加载](/zh/manual/textures#wait1)</li>
    <li>[等待多个纹理加载](/zh/manual/textures#waitmany)</li>
    <li>[从其他源加载纹理](/zh/manual/textures#cors)</li>
  </ul>
  <li>[内存使用](/zh/manual/textures#memory)</li>
  <li>[JPG vs PNG](/zh/manual/textures#format)</li>
  <li>[过滤和mips](/zh/manual/textures#filtering-and-mips)</li>
  <li>[重复，偏移，旋转，包裹](/zh/manual/textures#uvmanipulation)</li>
</ul>
...
我们将修改我们的第一个例子中的其中一个。我们需要做的就是创建一个[`TextureLoader`](/api/zh/loaders/TextureLoader)。调用它的[`load`](/api/zh/loaders/TextureLoader#load)方法，同时传入图像的URL，并将材质的 `map` 属性设置为该方法的返回值，而不是设置它的 `color`属性。
...
注意，我们使用的是 [`MeshBasicMaterial`](/api/zh/materials/MeshBasicMaterial)， 所以没有必要增加光线
...
我们只需制作6种材料，并在创建 [`Mesh`](/api/zh/objects/Mesh) 时将它们作为一个数组传递给它们。
...
但需要注意的是，并不是所有的几何体类型都支持多种材质。[`BoxGeometry`](/api/zh/geometries/BoxGeometry) 和 [`BoxGeometry`](/api/zh/geometries/BoxGeometry) 可以使用6种材料，每个面一个。[`ConeGeometry`](/api/zh/geometries/ConeGeometry) 和 [`ConeGeometry`](/api/zh/geometries/ConeGeometry) 可以使用2种材料，一种用于底部，一种用于侧面。 [`CylinderGeometry`](/api/zh/geometries/CylinderGeometry) 和 [`CylinderGeometry`](/api/zh/geometries/CylinderGeometry) 可以使用3种材料，分别是底部、顶部和侧面。对于其他情况，你需要建立或加载自定义几何体和（或）修改纹理坐标。
...
本文的大部分代码都使用最简单的加载纹理的方法。我们创建一个 [`TextureLoader`](/api/zh/loaders/TextureLoader) ，然后调用它的[`load`](/api/zh/loaders/TextureLoader#load)方法。
这将返回一个 [`Texture`](/api/zh/textures/Texture) 对象。
...
要等到所有纹理都加载完毕，你可以使用 [`LoadingManager`](/api/zh/loaders/managers/LoadingManager) 。创建一个并将其传递给 [`TextureLoader`](/api/zh/loaders/TextureLoader)，然后将其[`onLoad`](/api/zh/loaders/managers/LoadingManager#onLoad)属性设置为回调。
...
[`LoadingManager`](/api/zh/loaders/managers/LoadingManager) 也有一个 [`onProgress`](/api/zh/loaders/managers/LoadingManager#onProgress) 属性，我们可以设置为另一个回调来显示进度指示器。
...
纹理往往是three.js应用中使用内存最多的部分。重要的是要明白，_一般来说_，纹理会占用 `宽度 * 高度 * 4 * 1.33` 字节的内存。
...
在three.js中，这张图片会占用<strong>60兆（meg）的内存！</strong>。只要几个这样的纹理，你就会用完内存。
...
当纹理绘制的尺寸大于其原始尺寸时，你可以将 [`texture.magFilter`](/api/zh/textures/Texture#magFilter) 属性设置为 `THREE.NearestFilter` 或 `THREE.LinearFilter` 。`NearestFilter` 意味着只需从原始纹理中选取最接近的一个像素。对于低分辨率的纹理，这给你一个非常像素化的外观，就像Minecraft。
...
为了在绘制的纹理小于其原始尺寸时设置过滤器，你可以将 [`texture.minFilter`](/api/zh/textures/Texture#minFilter) 属性设置为下面6个值之一。
...
默认情况下，three.js中的纹理是不重复的。要设置纹理是否重复，有2个属性，[`wrapS`](/api/zh/textures/Texture#wrapS) 用于水平包裹，[`wrapT`](/api/zh/textures/Texture#wrapT) 用于垂直包裹。
...
最后需要注意的是，如果你改变了纹理上的 `wrapS` 或 `wrapT`，你还必须设置 [`texture.needsUpdate`](/api/zh/textures/Texture#needsUpdate)，以便three.js知道并应用这些设置。其他的设置会自动应用。
