...
three.js的优化有很多种方式. 常见的一种叫做*合并几何体*. 每一个你创建的[`Mesh`](/api/zh/objects/Mesh)代表一个(或多个)请求系统渲染的命令. 即便是画出来的结果一样, 画两个几何体总是比画一个要费时费力. 所以最好的方式就是将这些mesh合并起来.
...
每一个数据我们都创建了一个[`MeshBasicMaterial`](/api/zh/materials/MeshBasicMaterial)和一个[`Mesh`](/api/zh/objects/Mesh), 然后我们从位置辅助器中取得world matrix并应用到新的[`Mesh`](/api/zh/objects/Mesh)上. 最后, 我们在它的新位置上缩放.
...
最后, 我们将所有几何体的数组传入`BufferGeometryUtils.mergeGeometries`, 这个方法将会将其合并到一个mesh中
