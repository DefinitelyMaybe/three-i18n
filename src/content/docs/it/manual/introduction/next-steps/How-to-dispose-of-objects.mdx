---
title: Come liberare le risorse
sidebar:
  order: 1
---
...
Ogni volta che viene creata un'istanza di un tipo _three.js_, viene allocata una certa quantità di memoria. Tuttavia, _three.js_ crea per oggetti specifici
...
Una geometria solitamente rappresenta le informazioni sui vertici come una collezione di attributi. _three.js_ internamente crea un oggetto di tipo [ WebGLBuffer](https://developer.mozilla.org/en-US/docs/Web/API/WebGLBuffer)
per ogni attributo. Queste entità vengono cancellate solo se viene chiamato il metodo [BufferGeometry](api/core/BufferGeometry#dispose)(). Se una geometria, nella tua applicazione,
diventa obsoleta, esegui questo metodo per liberare tutte le risorse correlate.

## Materiali

Un materiale definisce come sono visualizzati gli oggetti. _three.js_ utilizza le informazioni di una definizione di materiale per costruire un programma shader per il rendering.
I programmi shader possono essere cancellati solo se il respettivo materiale è eliminato. Per ragioni di prestazioni, _three.js_ prova, se possibile, a riutilizzare
...
di un materiale eseguendo il metodo [Material](api/materials/Material#dispose)().
...
Ogni volta che crei un'istanza di [Texture](api/textures/Texture), three.js internamente crea un'istanza di [ WebGLTexture](https://developer.mozilla.org/en-US/docs/Web/API/WebGLTexture).
Come per i buffer, questo oggetto può essere eliminato solo chiamando il metodo[Texture](api/textures/Texture#dispose)().

Se stai usando `ImageBitmap` come origine dati della texture, devi chiamare il metodo [ ImageBitmap.close](https://developer.mozilla.org/en-US/docs/Web/API/ImageBitmap/close)() a livello applicativo per liberare le risorse lato CPU.
Una chiamata automatica di `ImageBitmap.close()`in [Texture](api/textures/Texture#dispose)() non è possibile, poiché il bitmap dell'immagine diventa inutilizzabile, e l'engine non ha modo di sapere se il bitmap dell'immagine è utilizzata da altre parti.

## Render Target

Oggetti di tipo [WebGLRenderTarget](api/renderers/WebGLRenderTarget) non allocano solo un'istanza di [ WebGLTexture](https://developer.mozilla.org/en-US/docs/Web/API/WebGLTexture) ma anche
di [ WebGLFramebuffer](https://developer.mozilla.org/en-US/docs/Web/API/WebGLFramebuffer) e di [ WebGLRenderbuffer](https://developer.mozilla.org/en-US/docs/Web/API/WebGLRenderbuffer)
per realizzare destinazioni di rendering personalizzate. Questi oggetti vengono solo deallocati eseguendo il metodo [WebGLRenderTarget](api/renderers/WebGLRenderTarget#dispose)().
...
### Perché _three.js_ non può eliminare gli oggetti automaticamente?

Questa domanda è stata fatta spesso dalla community, quindi è importante chiarire la questione. Il fatto è che _three.js_ non conosce il ciclo di vita o lo scopo
...
No, devi eliminare esplicitamente la geometria e il materiale chiamando il metodo _dispose()_. Tieni a mente che le geometrie e i materiali possono essere condivisi tra oggetti 3D come le mesh.

### _three.js_ fornisce informazioni sulla quantità di oggetti memorizzati nella cache?

Si, è possibile consultare [WebGLRenderer](api/renderers/WebGLRenderer#info). Una proprietà speciale del renderer con una serie di informazioni statistiche sulla memoria della scheda grafica
...
### Come devo gestire gli oggetti _three.js_ nella mia app? Quando so che devo eliminare le risorse?
