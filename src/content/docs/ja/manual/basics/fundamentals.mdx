...
古いブラウザーで実行したい場合は、[Babel](https://babeljs.io) のようなトランスパイラーを調べて下さい。
...
canvasを検索した後、[`WebGLRenderer`](/api/ja/renderers/WebGLRenderer) を作成します。
rendererは提供された全てのデータを取得し、canvasにレンダリングします。

今はWebGLを使用して3Dをcanvasにレンダリングする [`WebGLRenderer`](/api/ja/renderers/WebGLRenderer) があります。
...
これらの4つの設定は _"錐台"_ を定義します。
_錐台_ は、先端が切り取られたピラミッドのような3D形状の名前です。
...
次は [`Scene`](/api/ja/scenes/Scene) を作成します。
three.jsの [`Scene`](/api/ja/scenes/Scene) は、シーングラフのフォームのルートです。
...
次はボックスのデータが含まれている [`BoxGeometry`](/api/ja/geometries/BoxGeometry) を作成します。
...
次に [`Mesh`](/api/ja/objects/Mesh) を作成します。 [`Mesh`](/api/ja/objects/Mesh) は `Geometry` (オブジェクトの形状)と [`Material`](/api/ja/materials/Material) （オブジェクトの描画方法、光沢または平坦、適用する色、適用するテクステャなど）を組み合わせます。
...
また、マテリアルを変更する必要があります。[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial) はライトの影響を受けません。ライトの影響をうける [`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial) に変更してみましょう。
...
次に、異なる色とX位置を指定した [`Mesh`](/api/ja/objects/Mesh) インスタンスを配列に保存する関数を3回呼び出します。
...
バージョンr106以降でthree.jsを使用する好ましい方法は
[es6モジュール](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import)です。

es6モジュールはスクリプトのロードに `import` を使う事ができます。
また、HTMLのインラインに `<script type="module">` タグが使えます。
両方とも以下に例を示します。
...
このフォルダー構造が必要な理由は、 [`OrbitControls.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/controls/OrbitControls.js) のようなexamplesのスクリプトには相対パスがハードコーディングされてるからです。
