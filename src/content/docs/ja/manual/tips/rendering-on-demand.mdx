...
言い換えると `requestAnimationFrame` ループ、または"_rAF loop_"ループは以下のようになります。
...
最初に [`OrbitControls`](/examples/controls/OrbitControls) を追加します。これで何かの変更を反映してレンダリングする事ができます。
...
[`OrbitControls`](/examples/controls/OrbitControls) がカメラ設定を変更する時はレンダリングする必要があります。
幸いな事に [`OrbitControls`](/examples/controls/OrbitControls) は何か変更された時に `change` イベントをdispatchします。
...
[`OrbitControls`](/examples/controls/OrbitControls) には慣性のようなものを追加して動きを滑らかにするオプションがあります。
...
これで動きを滑らかにする新しいカメラ設定を [`OrbitControls`](/examples/controls/OrbitControls) に与えてくれます。
