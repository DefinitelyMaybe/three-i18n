...
[`WebGLRenderer`](/api/ja/renderers/WebGLRenderer) 作成時に `preserveDrawingBuffer: true` を渡す必要があります。
...
キャンバスを透明にしたい場合は [`WebGLRenderer`](/api/ja/renderers/WebGLRenderer) 作成時に[`alpha:true`](/api/ja/renderers/WebGLRenderer#alpha)を指定します。
...
また、プリマルチプライドアルファを<strong>使用しない</strong>事を指定したいでしょう。
...
Three.jsのデフォルトではキャンバスは[`premultipliedAlpha: true`](/api/ja/renderers/WebGLRenderer#premultipliedAlpha)で出力されますが、マテリアルは[`premultipliedAlpha: false`](/api/ja/materials/Material#premultipliedAlpha)で出力されます。
...
<ul>
  <li>以下のようにCSSでキャンバスの `position` を `fixed` にします。</li>
</ul>
