...
これを<strong>rtA</strong>と<strong>rtB</strong>と呼ぶ事にしましょう。
...
`RenderPass`に渡されたシーンは、まず<strong>rtA</strong>にレンダリングされ<strong>rtA</strong>は次のPassに渡されます。
...
最後に [`WebGLRenderer.render`](/api/ja/renderers/WebGLRenderer.render) の代わりに `EffectComposer.render` を使用し、`EffectComposer` にキャンバスのサイズを合わせます。
...
[`BloomPass.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/postprocessing/BloomPass.js)の中でこの行を見つけました。
...
同様に[`FilmPass.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/postprocessing/FilmPass.js)でこの行を見つけました。
...
`color` を Three.jsの [`Color`](/api/ja/math/Color) として宣言します。
...
もう1つの素晴らしいリソースは、[THREE.jsレポートの既存ポストプロセスシェーダーを読み解く](https://github.com/mrdoob/three.js/tree/master/examples/jsm/shaders)事です。
...
[エフェクト自体のコード](https://github.com/mrdoob/three.js/tree/master/examples/jsm/postprocessing)を読んで下さい。
