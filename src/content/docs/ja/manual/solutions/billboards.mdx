...
[前回のページでは](canvas-textures.html) [`CanvasTexture`](/api/ja/textures/CanvasTexture)を使ってラベルとバッジを作りました。バッジなどが常にカメラの方向を向いて文字が読める状態になっているという効果が欲しい時があります（訳註：ビルボード効果と言います）。Three.jsは[`Sprite`](/api/ja/objects/Sprite)と[`SpriteMaterial`](/api/ja/materials/SpriteMaterial) を使ってビルボード効果を実現できます。

In [a previous article](canvas-textures.html) we used a [`CanvasTexture`](/api/ja/textures/CanvasTexture)
to make labels / badges on characters. Sometimes we'd like to make labels or
other things that always face the camera. Three.js provides the [`Sprite`](/api/ja/objects/Sprite) and
[`SpriteMaterial`](/api/ja/materials/SpriteMaterial) to make this happen.

[この記事](canvas-textures.html)からサンプルを拝借して[`Sprite`](/api/ja/objects/Sprite)と[`SpriteMaterial`](/api/ja/materials/SpriteMaterial)を使ってみましょう。
...
<iframe src="/editor?url=/manual/examples/billboard-labels-w-sprites.html"></iframe>

しかし角度によってはラベルがオブジェクトに食い込んでしまうことがあります。

<div class="threejs_center">
  ![image](/resources/images/billboard-label-z-issue.png)
</div>
...
<iframe src="/editor?url=/manual/examples/billboard-labels-w-sprites-adjust-height.html"></iframe>
...
草の部分と幹の部分をそれぞれ[`Mesh`](/api/ja/objects/Mesh)で作り親オブジェクト[`Object3D`](/api/ja/core/Object3D)に加えます。
...
<iframe src="/editor?url=/manual/examples/billboard-trees-no-billboards.html"></iframe>
...
<iframe src="/editor?url=/manual/examples/billboard-trees-static-billboards.html"></iframe>
