...
そして、何も描画していない場所が透明になるように [`WebGLRenderer`](/api/ja/renderers/WebGLRenderer) に `alpha` を指定します。
...
描画には通常の [`PerspectiveCamera`](/api/ja/cameras/PerspectiveCamera) を使います。[`OrthographicCamera`](/api/ja/cameras/OrthographicCamera) は必要ないです。

もう1つの解決策は _キューブマップ_ を使用する事です。
...
[`CubeTextureLoader`](/api/ja/loaders/CubeTextureLoader) を使用してキューブマップ画像を読み込み、シーンの背景として使用します。
...
たいして変わらないですね。
まず、正距円筒図をテクスチャとして読み込み、読み込み後のコールバックで [`WebGLCubeRenderTarget.fromEquirectangularTexture`](/api/ja/renderers/WebGLCubeRenderTarget.fromEquirectangularTexture) を呼び出して、正距円筒図テクスチャからキューブマップを生成します。
