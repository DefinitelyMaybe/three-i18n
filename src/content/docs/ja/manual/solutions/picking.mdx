...
もちろん、マウスを _down_ した時だけこの関数を呼べますが、今回は全てのフレームでマウスの下に何かあるかピックします。
...
これを行うには [`PerspectiveCamera.setViewOffset`](/api/ja/cameras/PerspectiveCamera.setViewOffset) を使用します。
これでカメラを計算できます。
これで時間を節約できるはずです。
...
ここでは [`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial) を悪用している事に注意して下さい。
...
[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)を悪用する事は、最良の解決策でない事に注意して下さい。
...
[レンダーターゲットの記事](rendertargets.html)で説明したように [`WebGLRenderTarget`](/api/ja/renderers/WebGLRenderTarget) を使います。
