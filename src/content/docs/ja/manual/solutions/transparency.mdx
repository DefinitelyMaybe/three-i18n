...
キューブを透過するには [`transparent`](/api/ja/materials/Material#transparent)フラグを設定し、[`opacity`](/api/ja/materials/Material#opacity)を設定する必要があります。opacityは1は完全な不透明、0は完全な透明です。
...
[マテリアルの記事](materials.html)でマテリアルのプロパティ[`side`](/api/ja/materials/Material#side)について学びました。
...
Three.jsでは [`Mesh`](/api/ja/objects/Mesh) のようなオブジェクトに対してこれを行います。
...
ここでは [`PlaneGeometry`](/api/ja/geometries/PlaneGeometry) を使用していますが、デフォルトではテクスチャを平面全体に引き伸ばします。
[テクステャの記事で説明](textures.html)したように [`texture.repeat`](/api/ja/textures/Texture#repeat) と [`texture.offset`](/api/ja/textures/Texture#offset) を設定し、
各平面上の表面のテクスチャから正しい半分が得られるようにテクスチャを拡大縮小したり移動させたりできます。

上記のコードでは [`Object3D`](/api/ja/core/Object3D) を作成し、その親となる2つの平面を作成しています。
親の [`Object3D`](/api/ja/core/Object3D) を回転させた方が必要な計算するよりも簡単そうです。
...
交差する2つの平面（分割する前）に戻り、これらのテクスチャを使用して [`alphaTest`](/api/ja/materials/Material#alphaTest) を設定してみましょう。
