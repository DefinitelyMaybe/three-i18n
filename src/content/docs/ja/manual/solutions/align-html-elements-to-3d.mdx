...
[プリミティブの記事](primitives.html)を見ると [`TextGeometry`](/api/ja/geometries/TextGeometry) があり、3Dテキストが作れます。
...
[ライティングの記事](lights.html)のように [`OrbitControls`](/examples/controls/OrbitControls) を追加します。
...
カメラから地球儀上のその位置までのカメラの相対的な方向を取得し、_内積_ を取得します。
