...
これを行う方法の1つは [`CanvasTexture`](/api/ja/textures/CanvasTexture) を使用する事です。
...
画像を読込するコードを削除します。
代わりにキャンバスを作成し [`CanvasTexture`](/api/ja/textures/CanvasTexture) を作成してキャンバスに渡します。
...
唯一の余計な事は [`CanvasTexture`](/api/ja/textures/CanvasTexture) の `needsUpdate` プロパティを設定し、Three.jsにキャンバスの最新のコンテンツでテクスチャを更新する事です。
...
上記のようにルートの [`Object3D`](/api/ja/core/Object3D) に体、頭、ラベルを配置して位置を調整しています。
...
残作業はカメラを動かせるように [`OrbitControls`](/examples/controls/OrbitControls) を追加します。
