---
title: Web WorkerでOffscreenCanvasを使用する
sidebar:
  order: 2
---
...
2番目の引数は任意でWorkerに _転送_ したい最初のオブジェクトです。
このオブジェクトはクローンされません。
その代わりに _転送_ され、メインページには存在しなくなります。
...
もう1歩踏み込んで [`OrbitControls`](/examples/controls/OrbitControls) を追加してみましょう。
これはもう少し複雑です。
[`OrbitControls`](/examples/controls/OrbitControls) はマウス、タッチイベント、キーボードなどDOMをかなり広範囲にチェックしています。
...
これは [`EventDispatcher`](/api/ja/core/EventDispatcher) を継承しており、DOM要素のように `addEventListener` や `removeEventListener` のようなメソッドを提供しているので、OrbitControlsに渡せば動作するはずです。
...
共有のthree.jsコードでは [`OrbitControls`](/examples/controls/OrbitControls) をインポートして設定する必要があります。
...
これで動くと思われるが、実際に試してみると [`OrbitControls`](/examples/controls/OrbitControls) がもう少し必要なものがあると分かります。
...
これで [`OrbitControls`](/examples/controls/OrbitControls) が期待に沿った検査を行うための機能を提供します。

難しいのは分かっていますが手短に言うと:

`ElementProxy` はメインページ上で動作し、DOMイベントを転送します。
Worker内の `ElementProxyReceiver` は一緒に使うことができる `HTMLElement` を装っています。
[`OrbitControls`](/examples/controls/OrbitControls) と独自のコードを使用しています。
