...
Three.jsは _シャドウマップ_ をデフォルトで使用してます。
シャドウマップを機能させるには、_全てのライトにシャドウを落とし、光源に対して全てのオブジェクトもシャドウを落としてレンダリングします_。
急ぐ必要はないので <strong> もう一度読んでみて下さい！ </strong>
...
同じチェッカーボードの地面を使いますが、今回の地面には照明は必要ないので [`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial) を使用します。
...
そして、たくさんの球体を作ります。
各球体に対して [`THREE.Object3D`](/api/ja/core/Object3D) を作成し `base` に格納しシャドウの平面と球体メッシュの両方をbaseの子にします。
...
このシャドウは照明が不要なので [`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial) にします。
...
2つのライトを設定しました。
1つは [`HemisphereLight`](/api/ja/lights/HemisphereLight) で強度2にしました。
...
もう1つは [`DirectionalLight`](/api/ja/lights/DirectionalLight) で球体はいくつかの定義を得られます。
...
baseをxz平面内で移動させて [`Math.abs(Math.sin(time))`](</api/ja/math/Math.abs(Math.sin(time))>) で球体を上下に移動させると弾むようなアニメーションします。
...
[`DirectionalLight`](/api/ja/lights/DirectionalLight) と [`PointLight`](/api/ja/lights/PointLight) と [`SpotLight`](/api/ja/lights/SpotLight) です。

まずは、[ライトの記事](lights.html)のヘルパーの例を参考に [`DirectionalLight`](/api/ja/lights/DirectionalLight) を使ってみましょう。
...
この場合、[`DirectionalLight`](/api/ja/lights/DirectionalLight) にカメラがあり、ターゲットを見ています。
[以前取り上げたカメラと同じように](cameras.html)
ライトのシャドウカメラは影がレンダリングされ、内部の領域を定義します。
上記の例ではその面積が小さすぎます。

その領域を可視化するために、ライトのシャドウカメラを取得して [`CameraHelper`](/api/ja/helpers/CameraHelper) をシーンに追加します。
...
`DirectionLight` は全ての光が平行な方向に進むので、[`DirectionalLight`](/api/ja/lights/DirectionalLight) はシャドウカメラに [`OrthographicCamera`](/api/ja/cameras/OrthographicCamera) を使います。
[以前のカメラの記事](cameras.html)で [`OrthographicCamera`](/api/ja/cameras/OrthographicCamera) がどのように動作するかを説明しました。

[`OrthographicCamera`](/api/ja/cameras/OrthographicCamera) は、`left`、`right`、`top`、`bottom`、`near`、`far`、`zoom` プロパティでその箱、または _錐台の視点_ を定義してる事を思い出して下さい。
...
[`renderer.capabilities.maxTextureSize`](/api/ja/renderers/WebGLRenderer#capabilities)で利用可能な最大テクスチャサイズがわかります。

[`SpotLight`](/api/ja/lights/SpotLight) に切り替えると、ライトのシャドウカメラは [`PerspectiveCamera`](/api/ja/cameras/PerspectiveCamera) になります。
[`DirectionalLight`](/api/ja/lights/DirectionalLight) のシャドウカメラの設定を手動で行えます。
ただ、[`SpotLight`](/api/ja/lights/SpotLight) のシャドウカメラは [`SpotLight`](/api/ja/lights/SpotLight) 自身によって制御されます。
シャドウカメラの `fov` は [`SpotLight`](/api/ja/lights/SpotLight) の `angle` に接続しています。
...
そして最後に [`PointLight`](/api/ja/lights/PointLight) でシャドウをつけます。
[`PointLight`](/api/ja/lights/PointLight) は全方向に光を放つので関連する設定は `near` と `far` だけです。
それ以外の場合、[`PointLight`](/api/ja/lights/PointLight) のシャドウは、効果的な6つの [`SpotLight`](/api/ja/lights/SpotLight) のシャドウになります。
これは [`PointLight`](/api/ja/lights/PointLight) のシャドウの描画が非常に遅くなります。
...
そして、ライトを [`PointLight`](/api/ja/lights/PointLight) に切り替えます。
