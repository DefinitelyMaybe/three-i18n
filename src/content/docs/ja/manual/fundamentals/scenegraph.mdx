...
ここで[`Object3D`](/api/ja/core/Object3D)を作りました。[`Mesh`](/api/ja/objects/Mesh)のように、シーングラフのノードですが、[`Mesh`](/api/ja/objects/Mesh)とは異なり、マテリアルやジオメトリを持ちません。
ただローカルな空間を表現するだけです。
...
再び、描画されないシーングラフのノードを追加しました。これは、`earthOrbit`と呼ばれる[`Object3D`](/api/ja/core/Object3D)です。
...
一つは[`AxesHelper`](/api/ja/helpers/AxesHelper)です。
...
そのついでに、[`GridHelper`](/api/ja/helpers/GridHelper) というヘルパー関数も追加しておきましょう。
...
それぞれのノードに対して、[`GridHelper`](/api/ja/helpers/GridHelper)と[`AxesHelper`](/api/ja/helpers/AxesHelper)の両方を作りたいです。
...
内部的には各ノードに[`AxesHelper`](/api/ja/helpers/AxesHelper)と[`GridHelper`](/api/ja/helpers/GridHelper)の両方のvisible属性を設定することができます。
...
注意することは、[`AxesHelper`](/api/ja/helpers/AxesHelper)の`renderOrder`を2に設定し、[`GridHelper`](/api/ja/helpers/GridHelper)には1を設定することです。
...
これがシーングラフです。メッシュは緑色、[`Object3D`](/api/ja/core/Object3D)は青色、明かりは金色、カメラは紫色です。
...
ターゲット、つまり戦車が狙っているものとして、`targetOrbit`([`Object3D`](/api/ja/core/Object3D)) があります。
これはちょうど前述の`earthOrbit`と同じように回転します。
`targetOrbit`の子要素である`targetElevation` ([`Object3D`](/api/ja/core/Object3D))は、
`targetOrbit`からのオフセットと基準となる高さを提供します。
この子要素には、`targetElevation`に対して相対的に浮き沈みする、`targetBob`と呼ばれる[`Object3D`](/api/ja/core/Object3D)があります。
...
戦車には、`tank`と呼ばれる[`Object3D`](/api/ja/core/Object3D)があります。
これを使って戦車の子要素をすべて移動させることができます。
コードでは[`SplineCurve`](/api/ja/extras/curves/SplineCurve)を使っています。これは曲線に沿った位置を求めることができます。
0.0は曲線の始点です。1.0は曲線の終点です。これにより、戦車がある現在地を求めます。
次に、カーブの少し下の位置を求めて、[`Object3D.lookAt`](/api/ja/core/Object3D.lookAt)を使い、戦車をその方向に向けます。
...
ターゲットの方を向かせるのに、ターゲットの位置を求め、次に再び[`Object3D.lookAt`](/api/ja/core/Object3D.lookAt)を使うだけです。
...
[`Object3D`](/api/ja/core/Object3D)ノードを作り、物体をその子要素にすることは、three.jsのような3Dエンジンを上手く使うために
重要なステップです。
