...
<ul>
  <li>[ハロー・テクスチャ](/ja/manual/textures#hello)</li>
  <li>
    [立方体の各面に異なる6つのテクスチャを貼り付ける](/ja/manual/textures#six)
  </li>
  <li>[テクスチャの読み込み](/ja/manual/textures#loading)</li>
  <ul>
    <li>[簡単な方法](/ja/manual/textures#easy)</li>
    <li>[テクスチャの読み込みを待つ](/ja/manual/textures#wait1)</li>
    <li>[複数テクスチャの読み込みを待つ](/ja/manual/textures#waitmany)</li>
    <li>
      [異なるオリジンからのテクスチャの読み込み](/ja/manual/textures#cors)
    </li>
  </ul>
  <li>[メモリ使用](/ja/manual/textures#memory)</li>
  <li>[JPG vs PNG](/ja/manual/textures#format)</li>
  <li>[フィルタリングとMIP](/ja/manual/textures#filtering-and-mips)</li>
  <li>
    [テクスチャの繰り返し、オフセット、回転、ラッピング](/ja/manual/textures#uvmanipulation)
  </li>
</ul>

## <a name="hello"></a> ハロー・テクスチャ

テクスチャは\_一般的に\_PhotoshopやGIMPのような3rdパーティーのプログラムで最もよく作られる画像です。
...
最初の例を修正してみましょう。[`TextureLoader`](/api/ja/loaders/TextureLoader)を作ることで、必要なことはすべてできます。
[`load`](/api/ja/loaders/TextureLoader#load)を画像のURLを引数にして呼び、`color`を設定する代わりに、
...
[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial)を使っているので、光源が必要ないことに注意してください。
...
[`Mesh`](/api/ja/objects/Mesh)を作るときに、単に6つのマテリアルを作り、配列として渡します。
...
[`BoxGeometry`](/api/ja/geometries/BoxGeometry)と[`BoxGeometry`](/api/ja/geometries/BoxGeometry)は、それぞれの面に6つのマテリアルを使えます。
[`ConeGeometry`](/api/ja/geometries/ConeGeometry)と[`ConeGeometry`](/api/ja/geometries/ConeGeometry)は2つのマテリアルを使うことができ、一つは底面、一つは円錐面に適用されます。
[`CylinderGeometry`](/api/ja/geometries/CylinderGeometry)と[`CylinderGeometry`](/api/ja/geometries/CylinderGeometry)は3つのマテリアルを使うことができ、一つは底面、一つは上面、一つは側面に適用されます。
...
[`TextureLoader`](/api/ja/loaders/TextureLoader)を作り、その[`load`](/api/ja/loaders/TextureLoader#load)メソッドを呼びます。
これは[`Texture`](/api/ja/textures/Texture)オブジェクトを返します。
...
冒頭の例に戻り、このように、[`Mesh`](/api/ja/objects/Mesh)を作りシーンに追加する前に、テクスチャの読み込みを待つことができます。
...
全てのテクスチャが読み込まれたことを待つために、[`LoadingManager`](/api/ja/loaders/managers/LoadingManager)を使うことができます。
[`TextureLoader`](/api/ja/loaders/TextureLoader)を渡すと、[`onLoad`](/api/ja/loaders/managers/LoadingManager#onLoad)属性がコールバックに設定されます。
...
[`LoadingManager`](/api/ja/loaders/managers/LoadingManager)は[`onProgress`](/api/ja/loaders/managers/LoadingManager#onProgress)属性もあり、
...
となり、three.jsの<strong>60メガのメモリ！</strong>を消費するでしょう。
...
テクスチャが元の大きさより大きく描かれたときのフィルタ設定として、[`texture.magFilter`](/api/ja/textures/Texture#magFilter)属性に`THREE.NearestFilter`か`THREE.LinearFilter`を設定することができます。
...
[`texture.minFilter`](/api/ja/textures/Texture#minFilter)属性を6つの値から一つ設定できます。
...
水平方向のラッピングに[`wrapS`](/api/ja/textures/Texture#wrapS)と、垂直方向のラッピングに[`wrapT`](/api/ja/textures/Texture#wrapT)です。
...
three.jsが設定の適用を知るために、[`texture.needsUpdate`](/api/ja/textures/Texture#needsUpdate)も設定しなければならないことです。ほかの設定は自動的に適用されます。
