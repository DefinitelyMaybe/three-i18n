...
[`THREE.Color`](/api/ja/math/Color)型の属性は設定方法が複数あることに注意してください。
...
[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial)は光源の影響を受けません。
[`MeshLambertMaterial`](/api/ja/materials/MeshLambertMaterial)は頂点でのみ光を計算します。
一方で、[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)は全てのピクセルで光を計算します。
[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)は、specularによるハイライトもサポートします。
...
[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)の`shininess`設定は特定のハイライトの*輝き*を決めます。デフォルトは30です。
...
[`MeshLambertMaterial`](/api/ja/materials/MeshLambertMaterial)か[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)のどちらかで、colorに対して`emissive`属性を設定し、
色を黒（phongなら`shininess`を0）に設定すると、ちょうど[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial)のように見えることに注意してください。
...
[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)は[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial)や[`MeshLambertMaterial`](/api/ja/materials/MeshLambertMaterial)と同じようにできるのに、なぜ3種もあるのでしょうか。
...
光源やspecularによるハイライトが不要なら、[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial)を使うこともできます。

[`MeshToonMaterial`](/api/ja/materials/MeshToonMaterial)は[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)に似ていますが、一点大きな違いがあります。
...
一つ目は[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial)です。[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)と[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial)の
最大の違いは、異なるパラメータを使っていることです。
[`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial)は`shininess`設定があります。
[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial)は`roughness`と`metalness`の2つの設定があります。

基本的に、[`roughness`](/api/ja/materials/MeshStandardMaterial#roughness)は`shininess`の逆です。
野球ボールがほとんど反射しないように、とても粗いのものがある一方で、
とても光沢があるビリヤード玉のように、粗くないものもあります。
roughnessは0から1の間をとります。

もう一つの設定で、[`metalness`](/api/ja/materials/MeshStandardMaterial#metalness)は、マテリアルの金属っぽさです。
金属は非金属と異なった振る舞いをします。
0は非金属で、1は金属です。

ここに、[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial)のサンプルがあります。
右に行くにつれて、`roughness`は0から1に変わります。
下に行くにつれて、`metalness`は0から1に変わります。

<div data-diagram="MeshStandardMaterial" style="min-height: 400px"></div>

[`MeshPhysicalMaterial`](/api/ja/materials/MeshPhysicalMaterial)は、[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial)と同様ですが、
...
[`MeshBasicMaterial`](/api/ja/materials/MeshBasicMaterial) ➡ [`MeshLambertMaterial`](/api/ja/materials/MeshLambertMaterial) ➡ [`MeshPhongMaterial`](/api/ja/materials/MeshPhongMaterial) ➡
[`MeshStandardMaterial`](/api/ja/materials/MeshStandardMaterial) ➡ [`MeshPhysicalMaterial`](/api/ja/materials/MeshPhysicalMaterial)になります。
低速なマテリアルは、より現実味のある見た目のシーンを作ることができますが、
パワーが低いデバイスやモバイル端末では、より高速なマテリアルを使うようにコードを設計する必要があります。

続いて、特別な用途に使う3つのマテリアルがあります。
[`ShadowMaterial`](/api/ja/materials/ShadowMaterial)は影から作られたデータを得るのに使われます。
まだ影については説明していませんでしたね。
その際には、このマテリアルを使って、シーンの裏で何が起きているのか、のぞいてみたいと思います。

[`MeshDepthMaterial`](/api/ja/materials/MeshDepthMaterial)は各ピクセルの深度を描写します。
カメラの負の[`near`](/api/ja/cameras/PerspectiveCamera#near)にあるピクセルは0、負の[`far`](/api/ja/cameras/PerspectiveCamera#far)にあるピクセルは1です。
...
[`MeshNormalMaterial`](/api/ja/materials/MeshNormalMaterial)はジオメトリの*法線*を表示します。
*法線*は、特定の三角形かピクセル表面の方向です。
[`MeshNormalMaterial`](/api/ja/materials/MeshNormalMaterial)は見えている空間の法線を描画します（法線はカメラに依存します）。
...
[`ShaderMaterial`](/api/ja/materials/ShaderMaterial)は、three.jsのシェーダーシステムを使ったカスタムマテリアルを作るためのものです
[`RawShaderMaterial`](/api/ja/materials/RawShaderMaterial)は、three.jsの補助なしで、完全に独自シェーダーを作るためのものです。
これらのトピックはどちらも大きいため、後ほど説明します。

全てのマテリアルは[`Material`](/api/ja/materials/Material)によって決められた設定を共有しています。
それらは[ドキュメントを見てください](/api/ja/materials/Material)。けれども、最も一般的に使われる2つの属性について説明しましょう。

[`flatShading`](/api/ja/materials/Material#flatShading)：物体の面が分割されて見えるか、滑らかに見えるか。デフォルトは`false`です。
...
[`side`](/api/ja/materials/Material#side)：三角形の両面を表示するか。デフォルトは`THREE.FrontSide`です。
...
変更するケースでは、three.jsに変更を伝えるため、`material.needsUpdate = true`を設定する必要があります。
マテリアルを試用した後で、`needsUpdate`の設定を必要とする一般的な設定はこのようになります：
...
`needsUpdate = true`を設定する必要があります。

テクスチャありからテクスチャなしに変更するケースでは、
1x1ピクセルのホワイトテクスチャを使うことがよいです。

</li>
</ul>

この問題は、ほとんどのアプリには関係ありません。
