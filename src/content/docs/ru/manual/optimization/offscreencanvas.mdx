---
title: Использование OffscreenCanvas в воркере
sidebar:
  order: 2
---
...
Сделаем еще один шаг и добавим [`OrbitControls`](/examples/controls/OrbitControls). Это будет немного больше.
[`OrbitControls`](/examples/controls/OrbitControls) довольно широко используют DOM для проверки мыши, событий касания и клавиатуры.

В отличие от нашего кода, мы не можем использовать объект глобального `state`, не переписав весь код [`OrbitControls`](/examples/controls/OrbitControls) для работы с ним. [`OrbitControls`](/examples/controls/OrbitControls) принимают элемент, к которому они присоединяют большинство используемых ими событий DOM. Возможно, мы могли бы передать наш собственный объект, имеющий ту же поверхность API, что и элемент DOM. Нам нужно только поддерживать функции, которые необходимы [`OrbitControls`](/examples/controls/OrbitControls).
...
Все, что он делает, - это если он получает сообщение, то отправляет его. Он наследуется от [`EventDispatcher`](/api/en/core/EventDispatcher), который предоставляет такие методы, как `addEventListener` и `removeEventListener`, точно так же, как элемент DOM, поэтому, если мы передадим его в [`OrbitControls`](/examples/controls/OrbitControls), он должен работать.
...
Нам также нужно добавить [`OrbitControls`](/examples/controls/OrbitControls) в начало скрипта.
...
Обратите внимание, что мы передаем [`OrbitControls`](/examples/controls/OrbitControls) нашему прокси через `inputElement` вместо передачи холста, как в других примерах, отличных от `OffscreenCanvas`.
...
Это кажется близким к запуску, но если мы действительно попробуем, то увидим, что [`OrbitControls`](/examples/controls/OrbitControls) нужно еще кое-что.
...
Еще несколько приемов. [`OrbitControls`](/examples/controls/OrbitControls) добавляют события `pointermove` и `pointerup` в `ownerDocument` элемента для обработки захвата мыши (когда мышь выходит за пределы окна).
...
Это даст [`OrbitControls`](/examples/controls/OrbitControls) возможность проверить, что соответствует их ожиданиям.
...
в воркере, который маскируется под `HTMLElement`, который мы можем использовать как с [`OrbitControls`](/examples/controls/OrbitControls), так и с нашим собственным кодом.
