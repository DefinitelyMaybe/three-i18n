...
<span style="color: blue;">Синяя полоса</span> обозначает `latHelper` который
используется для поворота на широту выше или ниже экватора.
<span style="color: red;">Красная сфера</span> представляет смещение, которое
обеспечивает этот `positionHelper`.
...
Для каждой точки данных мы создаем [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) и [`Mesh`](/api/en/objects/Mesh), а затем запрашиваем мировую матрицу
`positionHelper` и применяем ее к новой [`Mesh`](/api/en/objects/Mesh). Наконец мы масштабируем сетку в новой позиции.
