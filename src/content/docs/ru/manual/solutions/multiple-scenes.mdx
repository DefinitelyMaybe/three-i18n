---
title: Несколько холстов, несколько сцен
sidebar:
  order: 4
---
...
Для каждого холста сделайте [`Renderer`](/api/en/constants/Renderer).

Вы быстро обнаружите, что у вас возникли проблемы.

<ol>
<li>
Браузер ограничивает количество контекстов WebGL, которые вы можете иметь.
...
<li>
Ресурсы WebGL не могут быть разделены между контекстами (no shared memory)
...
Мы делаем один [`Renderer`](/api/en/constants/Renderer) и затем одну [`Scene`](/api/en/scenes/Scene) для каждого виртуального холста.
...
<p>
  ...
  Я люблю коробки (boxes). Подарки приходят в коробках. Когда я нахожу новую
  коробку, я всегда рад узнать, что внутри.
</p>
<p>
  <span id="pyramid" class="diagram right"></span>
  Когда я был ребенком, я мечтал отправиться в экспедицию внутри пирамиды
  (pyramid). и найти неоткрытую гробницу, полную мумий и сокровищ.
</p>
```
...
холста, включив тест _scissor_ через [`Renderer.setScissorTest`](/api/en/constants/Renderer.setScissorTest), затем установим
плоскость обрезания и область просмотра (Viewport) с помощью
[`Renderer.setViewport`](/api/en/constants/Renderer.setViewport) и [`Renderer.setScissor`](/api/en/constants/Renderer.setScissor).
...
И если мы [быстро прокрутим вверх и вниз](../examples/multiple-scenes-v2.html)
...
[ материал в середине страницы должен совпадать](../examples/multiple-scenes-v2.html),
...
Мы изменим код установки сцены, чтобы он представлял собой карту имен для _функций инициализации сцены_
, которые возвращают функцию _отрисовки сцены_.
