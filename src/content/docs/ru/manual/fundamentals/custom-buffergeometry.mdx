...
[`BufferGeometry`](/api/en/core/BufferGeometry) с другой стороны использует названный `BufferAttributes`.
Каждый атрибут [`BufferAttribute`](/api/en/core/BufferAttribute) представляет собой массив данных одного типа: позиции, нормали, цвета и ультрафиолетовые лучи.
...
Для каждой стороны тоже нужны разные ультрафиолеты. Это указывает на самую большую разницу между `Geometry` и [`BufferGeometry`](/api/en/core/BufferGeometry). Ничего общего с [`BufferGeometry`](/api/en/core/BufferGeometry).
...
Иногда это облегчает использование Geometry. С [`BufferGeometry`](/api/en/core/BufferGeometry) мы можем предоставить данные, уже преобразованные в этот формат.

В качестве простого примера давайте сделаем куб, используя [`BufferGeometry`](/api/en/core/BufferGeometry).
...
а затем преобразуем эти данные в параллельные массивы и, наконец, используем их для создания атрибутов `Buffer` и добавления их в [`BufferGeometry`](/api/en/core/BufferGeometry).
...
Наконец, мы можем создать [`BufferGeometry`](/api/en/core/BufferGeometry), а затем [`BufferAttribute`](/api/en/core/BufferAttribute) для каждого массива и добавить его в [`BufferGeometry`](/api/en/core/BufferGeometry).
...
типа Float32Array. Атрибут [`BufferAttribute`](/api/en/core/BufferAttribute) требует TypedArray, а не собственного массива. Атрибут [`BufferAttribute`](/api/en/core/BufferAttribute) также требует, чтобы вы указали,
...
Затем мы указываем 36 индексов для 36 вершин, которые нам нужно нарисовать, чтобы сделать 12 треугольников, вызывая [`BufferGeometry.setIndex`](/api/en/core/BufferGeometry.setIndex) с массивом индексов.
...
Как и в `Geometry`, в [`BufferGeometry`](/api/en/core/BufferGeometry) есть метод [`computeVertexNormals`](/api/en/core/BufferGeometry#computeVertexNormals) для вычисления нормалей,
...
[`BufferGeometry`](/api/en/core/BufferGeometry) напрямую для создания собственной геометрии и как динамически
обновлять содержимое [`BufferAttribute`](/api/en/core/BufferAttribute). То, что вы используете, `Geometry` или [`BufferGeometry`](/api/en/core/BufferGeometry),
