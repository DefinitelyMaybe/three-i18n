...
обратите внимание, что свойства типа [`THREE.Color`](/api/en/math/Color) могут быть установлены несколькими способами.
...
[`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) не зависит от света.
[`MeshLambertMaterial`](/api/en/materials/MeshLambertMaterial) вычисляет освещение только в вершинах vs, [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) который
вычисляет освещение в каждом пикселе и [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) также поддерживающий
...
`shininess` устанавливает [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) определяя _блеск_ от бликов. Значение по умолчанию - 30.
...
[`MeshLambertMaterial`](/api/en/materials/MeshLambertMaterial) или [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) и установка `color` в черный
(и `shininess` в 0 для Фонга) в конечном итоге будет выглядеть как [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial).
...
Зачем нам все 3, когда [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) может делать то же самое, что [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial)
и [`MeshLambertMaterial`](/api/en/materials/MeshLambertMaterial)? Причина в том, что более сложный материал требует больше ресурсов
...
блики, используйте [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial).

[`MeshToonMaterial`](/api/en/materials/MeshToonMaterial) похож на [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) с одной большой разницей.
...
Далее идут 2 _физически обоснованных_ материала. Physically Based
...
Первый - [`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial). Самая большая разница с
[`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) и [`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial) - использование различных параметров.
У [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) была `shininess` настройка. [`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial) имеет 2
настройки `roughness` и `metalness`.

По простому [шероховатость](https://en.wikipedia.org/wiki/Surface_roughness)
[`roughness`](/api/en/materials/MeshStandardMaterial#roughness) это противоположность
...
Другая настройка - [`metalness`](/api/en/materials/MeshStandardMaterial#metalness). Она говорит о том,
...
Вот краткий пример [`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial) с `roughness` от 0 до 1
поперёк и `metalness` от 0 до 1 вниз.

<div data-diagram="MeshStandardMaterial" style="min-height: 400px"></div>

[`MeshPhysicalMaterial`](/api/en/materials/MeshPhysicalMaterial) же самое, что и [`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial) но он добавляет `clearcoat`
...
[`MeshStandardMaterial`](/api/en/materials/MeshStandardMaterial) ➡ [`MeshPhysicalMaterial`](/api/en/materials/MeshPhysicalMaterial). Более медленные материалы
...
Есть 3 материала, которые имеют специальное использование. [`ShadowMaterial`](/api/en/materials/ShadowMaterial)
...
[`MeshDepthMaterial`](/api/en/materials/MeshDepthMaterial) отрисовывает глубину каждого пикселя, где пиксели при
отрицательном [`near`](/api/en/cameras/PerspectiveCamera#near) камеры равны 0
и при отрицательном [`far`](/api/en/cameras/PerspectiveCamera#far) равны 1.
...
The [`MeshNormalMaterial`](/api/en/materials/MeshNormalMaterial) Покажет вам _нормали_ геометрии.
_Нормали_ - это направление конкретного треугольника или грани пикселя.
[`MeshNormalMaterial`](/api/en/materials/MeshNormalMaterial) рисует пространство просмотра нормалей (нормали относительно камеры).
...
[`ShaderMaterial`](/api/en/materials/ShaderMaterial) предназначен для изготовления нестандартных материалов
с использованием шейдерной системы three.js.
[`RawShaderMaterial`](/api/en/materials/RawShaderMaterial) предназначен для создания полностью пользовательских
шейдеров без помощи three.js. Обе эти темы большие и будут рассмотрены позже.

Большинство материалов имеют множество настроек, определенных [`Material`](/api/en/materials/Material).
[Посмотрите документацию](/api/en/materials/Material) по ним, и давайте рассмотрим два
...
[`side`](/api/en/materials/Material#side): какие стороны треугольников показать. По умолчанию = `THREE.FrontSide`.
...
<ul>
  <li>
    для плоскостей или других нетвердых объектов, где обычно видны задние
...
В этих случаях вам нужно указать `material.needsUpdate = true` чтобы
three.js применил ваши существенные изменения. Наиболее распространенные настройки,
которые необходимо установить, `needsUpdate` если вы измените настройки
...
[
flat shaded
](https://en.wikipedia.org/wiki/Shading#Flat_shading) и не flat shaded.
