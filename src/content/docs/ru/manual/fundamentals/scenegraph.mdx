...
Мы создаем материал синего цвета, но мы дали ему небольшое количество _излучающего_
...
Здесь мы сделали [`Object3D`](/api/en/core/Object3D). Как и [`Mesh`](/api/en/objects/Mesh) он также является узлом в графе сцены,
но в отличие от [`Mesh`](/api/en/objects/Mesh) он не имеет материала или геометрии. Это просто
...
Снова мы добавили еще один невидимый узел графа сцены [`Object3D`](/api/en/core/Object3D) под названием `earthOrbit`
...
Один называется [`AxesHelper`](/api/en/helpers/AxesHelper). Он рисует 3 линии, представляющие локальные оси

<span style="color:red">X</span>,<span style="color:green">Y</span>, и
...
<span style="color:blue">z (синяя)</span> Поскольку мы смотрим прямо вниз, и
каждый из наших объектов вращается только вокруг своей оси y, мы не видим
большую часть осей <span style="color:green">y (зеленая)</span>.
...
давайте также добавим еще одного помощника под названием [`GridHelper`](/api/en/helpers/GridHelper).
...
Мы хотим сделать [`GridHelper`](/api/en/helpers/GridHelper) и [`AxesHelper`](/api/en/helpers/AxesHelper) для каждого узла. Нам нужна метка для каждого
...
свойство [`AxesHelper`](/api/en/helpers/AxesHelper) и [`GridHelper`](/api/en/helpers/GridHelper) для узла.
...
Мы устанавливаем `renderOrder` в [`AxesHelper`](/api/en/helpers/AxesHelper)
равным 2, а для [`GridHelper`](/api/en/helpers/GridHelper) равным 1 так, что оси втянуться после появления сетки.
...
_локальном пространстве_ `solarSystem`. Включите `earthOrbit` и вы увидите,
как луна ровно на 2 единицы от центра _локального пространства_ `earthOrbit`.
...
и как вы можете его использовать. Создание [`Object3D`](/api/en/core/Object3D) узлов и родительских
