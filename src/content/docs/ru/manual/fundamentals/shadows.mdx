...
Three.js по умолчанию использует _shadow maps_. Shadow map работает так: _для каждого источника света, отбрасывающего тени, все объекты,
помеченные для отбрасывания теней, визуализируются с точки зрения источника света_. <strong>ПРОЧИТАЙТЕ ЭТО СНОВА!</strong> и это запомнится.
...
Затем мы установим ту же самую шахматную доску, но на этот раз она использует [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial), так как нам не нужно освещение для земли.
...
Теперь мы сделаем кучу сфер. Для каждой сферы мы создадим `base` [`THREE.Object3D`](/api/en/core/Object3D) и сделаем сетку теневой плоскости и дочернюю сетку базовой сферы.
...
Тень - это [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial), потому что ей не нужно освещение.
...
Мы установили 2 источника света. Одним из них является [`HemisphereLight`](/api/en/lights/HemisphereLight) с интенсивностью, установленной на 2, чтобы действительно осветлить вещи.
...
Другой - [`DirectionalLight`](/api/en/lights/DirectionalLight), поэтому сферы получают некоторое определение.
...
используя [`Math.abs (Math.sin (time))`](/api/en/math/Math.abs "Math.sin (time")), который дает нам оживленную анимацию.
...
Итак, переходя к теневым картам, есть 3 источника света, которые могут отбрасывать тени. [`DirectionalLight`](/api/en/lights/DirectionalLight), [`PointLight`](/api/en/lights/PointLight) и [`SpotLight`](/api/en/lights/SpotLight).

Давайте начнем с [`DirectionalLight`](/api/en/lights/DirectionalLight) c вспомогательного примера из [статьи о светах](lights.html).
...
В этом случае в [`DirectionalLight`](/api/en/lights/DirectionalLight) есть камера, которая смотрит на свою цель. Точно так же, как [камера, которую мы ранее покрывали](cameras.html),
камера тени определяет область, внутри которой рендерится тени. В приведенном выше примере эта область слишком мала.

Чтобы визуализировать эту область, мы можем получить теневую камеру и добавить [`CameraHelper`](/api/en/helpers/CameraHelper) к сцене.
...
Поскольку [`DirectionalLight`](/api/en/lights/DirectionalLight) представляет свет, движущийся в параллельном направлении,
[`DirectionalLight`](/api/en/lights/DirectionalLight) использует [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) для своей теневой камеры. Мы рассмотрели, как работает
[`OrthographicCamera`](/api/en/cameras/OrthographicCamera) в [предыдущей статье о камерах](cameras.html).

Напомним, [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) определяет свою рамку или вид усечения по своим свойствам `left`, `right`, `top`, `bottom`, `near`, `far` и `zoom`.
...
размер текстуры, который доступен в рендере как [`renderer.capabilities.maxTextureSize`](/api/en/renderers/WebGLRenderer#capabilities).

При переключении на [`SpotLight`](/api/en/lights/SpotLight) теневая камера источника света становится [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera).
В отличие от теневой камеры [`DirectionalLight`](/api/en/lights/DirectionalLight), где мы могли вручную установить большинство ее настроек,
теневая камера SpotLight управляется самим [`SpotLight`](/api/en/lights/SpotLight). Поле зрения для теневой камеры напрямую связано с настройкой угла [`SpotLight`](/api/en/lights/SpotLight).
...
И, наконец, есть тени с [`PointLight`](/api/en/lights/PointLight). Так как [`PointLight`](/api/en/lights/PointLight) светит во всех направлениях, единственные релевантные настройки - ближние и дальние.
В противном случае тень [`PointLight`](/api/en/lights/PointLight) фактически представляет собой 6 теней [`SpotLight`](/api/en/lights/SpotLight), каждая из которых указывает на грань куба вокруг источника света.
Это означает, что тени [`PointLight`](/api/en/lights/PointLight) намного медленнее, поскольку вся сцена должна быть нарисована 6 раз, по одному для каждого направления.
...
И, конечно же, нам нужно переключить свет на [`PointLight`](/api/en/lights/PointLight).
