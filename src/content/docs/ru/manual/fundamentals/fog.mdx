...
В three.js вы добавляете туман, создавая объект [`Fog`](/api/en/scenes/Fog) или [`FogExp2`](/api/en/scenes/FogExp2) и устанавливая его в свойстве сцены
[`fog`](/api/en/scenes/Scene#fog).

[`Fog`](/api/en/scenes/Fog) позволяет выбирать `near` и `far` настройки, которые находятся на расстоянии от камеры.
...
Есть также [`FogExp2`](/api/en/scenes/FogExp2), который растет экспоненциально с расстоянием от камеры.
...
или для [`FogExp2`](/api/en/scenes/FogExp2) это будет
...
[`FogExp2`](/api/en/scenes/FogExp2) ближе к реальности, но [`Fog`](/api/en/scenes/Fog) используется чаще, поскольку он позволяет вам выбрать место для нанесения тумана,
...
Важно отметить, что туман применяется к _вещам, которые отображаются_.
...
Цвет фона устанавливается с помощью свойства [`scene.background`](/api/en/scenes/Scene#background). Чтобы выбрать цвет фона, вы прикрепляете к нему [`THREE.Color`](/api/en/math/Color). Например
...
`.Listen ()` в конце последних 2 строк указывает lil-gui _прослушивать_
...
Итак, давайте добавим еще одно _виртуальное_ свойство в наш помощник, который будет устанавливать оба цвета, когда lil-gui манипулирует им.
...
К счастью, [`THREE.Color`](/api/en/math/Color) как метод [`getHexString`](/api/en/math/Color#getHexString)
...
И последнее: на материале существует логическое свойство [`fog`](/api/en/materials/Material#fog),
