...
`requestAnimationFrame` или "_цикл RAF_" примерно так
...
Нам нужно рендерить каждый раз, когда [`OrbitControls`](/examples/controls/OrbitControls) меняет настройки камеры. К счастью, [`OrbitControls`](/examples/controls/OrbitControls) отправляет событие `change` каждый раз, когда что-то меняется.
...
У [`OrbitControls`](/examples/controls/OrbitControls) есть опции для добавления некоторой инерции, чтобы они чувствовали себя менее интенсивными.
...
чтобы [`OrbitControls`](/examples/controls/OrbitControls) продолжал предоставлять нам новые настройки камеры, поскольку они сглаживают движение.
