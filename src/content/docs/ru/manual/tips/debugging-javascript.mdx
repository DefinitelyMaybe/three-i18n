...
Внутри всех devtools есть _консоль_. Показывает предупреждения и сообщения об ошибках.

<strong> ЧИТАЙТЕ СООБЩЕНИЯ!! </strong>
...
Если вы видите какие-либо другие <strong>ПРОЧИТАЙТЕ ИХ</strong>. Например:
...
И давайте создадим простой класс для управления этим _буфером очистки_.
...
[статьи о загрузке файлов gLTF](load-gltf.html) я сделал кривую, используя класс [`SplineCurve`](/api/en/extras/curves/SplineCurve), который создает 2D-кривую.
...
В этом случае вторым аргументом является `car.position`, который является [`Vector3`](/api/en/math/Vector3).
Функция `set` в [`Vector3`](/api/en/math/Vector3) требует 3 аргумента, x, y и z, но [`SplineCurve`](/api/en/extras/curves/SplineCurve) является 2D-кривой и поэтому вызывает `car.position.set` только с x и y.
...
## Создание _Minimal, Complete, Verifiable, Example_ для Stack Overflow
...
<strong>Минимальная</strong> часть важна. Допустим, у вас возникла проблема с
перемещением пути в последнем примере [загрузки статьи gLTF](load-gltf.html).
...
Вы можете удалить источники света и тени с помощью [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial).
Вы, конечно, можете удалить код `lil-gui`. Код создает плоскость с текстурой. Было бы проще использовать [`GridHelper`](/api/en/helpers/GridHelper).
...
Также важно, когда вы отправляете свой вопрос в Stack Overflow, <strong>поместите свой код [во фрагмент](https://stackoverflow.blog/2014/09/16/introducing-runnable-javascript-css-and-html-code-snippets/).</strong>
...
## Используйте [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial)

Поскольку [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) не использует источники света, это один из способов устранения причин, по которым что-то может не отображаться.
Если ваши объекты отображаются с использованием [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial), но не с какими-либо материалами, которые вы использовали, то вы знаете,
...
[`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera) имеет ближние и дальние настройки, которые описаны в
...
Возможно, даже просто _временно_ установите для них что-то большое, например, `near` = 0,001 и `far` = 1000000.
...
как [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) и убедитесь, что вы можете сделать это на экране.
