---
title: Создание анимированного фона в three.js 
sidebar:
  order: 7
---
...
Вам нужно передать `preserveDrawingBuffer: true` при создании [`WebGLRenderer`](/api/en/renderers/WebGLRenderer).
...
передайте [`alpha:true`](/api/en/renderers/WebGLRenderer#alpha) при создании [`WebGLRenderer`](/api/en/renderers/WebGLRenderer)
...
[`premultipliedAlpha: true`](/api/en/renderers/WebGLRenderer#premultipliedAlpha) но по умолчанию используется для материалов, которые выводят
[`premultipliedAlpha: false`](/api/en/materials/Material#premultipliedAlpha).
...
<ul>
  <li>Установите у холста CSS `position` `fixed` как в </li>
</ul>
