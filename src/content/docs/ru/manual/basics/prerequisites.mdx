...
[_эта_ статья может помочь объяснить `this`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this).

## ES5/ES6/ES7 материал

### `var` устарела. Используйте `const` и/или `let`

Нет никакой причины использовать `var` <strong>КОГДА-НИБУДЬ</strong> и сейчас считается плохой практикой, чтобы использовать его вообще.
