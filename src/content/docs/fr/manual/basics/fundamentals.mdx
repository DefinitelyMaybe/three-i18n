...
un transpileur tel que [Babel](https://babeljs.io).
...
<ul>
<li>
Il y a un [`Renderer`](/api/en/constants/Renderer). C'est sans doute l'objet principal de Three.js. Vous passez
une [`Scene`](/api/en/scenes/Scene) et une [`Camera`](/api/en/cameras/Camera) à un [`Renderer`](/api/en/constants/Renderer) et il effectue le rendu (dessine) de la
partie de la scène 3D qui est à l'intérieur de l'espace visible (en réalité une pyramide tronquée ou _frustum_)
de la caméra dans une image 2D affichée dans un canevas (_canvas_).

</li>
<li>
Il y a un [graphe de scène](scenegraph.html) qui est une structure arborescente,
constituée de divers objets tel qu'un objet [`Scene`](/api/en/scenes/Scene), de multiple maillages ([`Mesh`](/api/en/objects/Mesh)),
des lumières ([`Light`](/api/en/lights/Light)), des groupes ([`Group`](/api/en/objects/Group)), des objets 3D [`Object3D`](/api/en/core/Object3D) et des objets [`Camera`](/api/en/cameras/Camera).
Un objet [`Scene`](/api/en/scenes/Scene) définit la racine d'un graphe de scène et contient des propriétés telles que
...
Il est à noter sur que ce diagramme [`Camera`](/api/en/cameras/Camera) est partiellement placé dans le graphe de scène.
Cela permet d'attirer l'attention qu'en Three.js, contrairement aux autres objets, une [`Camera`](/api/en/cameras/Camera) ne doit
pas forcément faire partie du graphe de scène pour être opérationnelle. Une [`Camera`](/api/en/cameras/Camera), de la même
...
de multiples objets [`Camera`](/api/en/cameras/Camera) dans un unique graphe de scène est donné en exemple.

</li>
<li>
Les objets de type [`Mesh`](/api/en/objects/Mesh) représentent une géométrie (`Geometry`) liée à un matériau ([`Material`](/api/en/materials/Material))
spécifique. Les objets [`Material`](/api/en/materials/Material) et `Geometry` peuvent être liés à plusieurs objets [`Mesh`](/api/en/objects/Mesh)
simultanément. Par exemple, pour dessiner deux cubes bleus à des positions différentes, nous
pouvons soit utiliser deux objets [`Mesh`](/api/en/objects/Mesh) pour spécifier les positions et orientations de
chaque cube; soit nous pouvons utiliser seulement une géométrie unique (`Geometry`) pour décrire les
données spatiales du cube et un matériau unique ([`Material`](/api/en/materials/Material)) pour spécifier la couleur bleue.
Les deux objets [`Mesh`](/api/en/objects/Mesh) peuvent ainsi référencer les mêmes objets `Geometry` et [`Material`](/api/en/materials/Material).
...
<li>
Les objets [`Material`](/api/en/materials/Material) représentent les
[propriétés de surface utilisées pour dessiner la géométrie](materials.html)
telles que la couleur à utiliser ou le pouvoir réfléchissant (brillance). Un matériau ([`Material`](/api/en/materials/Material))
peut aussi se référer à un ou plusieurs objets [`Texture`](/api/en/textures/Texture) dont l'utilité est, par exemple, de plaquer
une image sur la surface d'une géométrie.

</li>
<li>
Les objets [`Texture`](/api/en/textures/Texture) représentent généralement des images soit [chargées de fichiers image](textures.html),
soit [générées par le biais d'un canevas](canvas-textures.html) ou
[résultant du rendu d'une autre scène](rendertargets.html).

</li>
<li>
Les objets [`Light`](/api/en/lights/Light) représentent [différentes sortes de lumière](lights.html).

</li>
</ul>

Maintenant que tout cela a été défini, nous allons présenter un exemple de type _"Hello Cube"_ utilisant un
nombre minimum d'éléments Three.js :
...
Après la recherche du canevas, nous créons un [`WebGLRenderer`](/api/en/renderers/WebGLRenderer). Le _renderer_
...
Ensuite, nous avons besoin d'une caméra. Nous créons une [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera).
...
`aspect` est le ratio d'affichage dans le canevas. Cela sera détaillé
[dans un autre article](responsive.html). Toutefois, par défaut,
un canevas est de taille 300x150 pixels ce qui lui confère un ratio de 300/150 ou 2.

`near` et `far` délimitent la portion de l'espace devant la caméra dont
le rendu est effectué. Tout ce qui est avant ou après est découpé (_clipped_),
donc non dessiné.

Ces 4 paramètres définissent une pyramide tronquée ou _"frustum"_.
En d'autres termes, il s'agit d'une autre forme 3D à l'instar des sphères,
cubes et prismes.

![image](/resources/frustum-3d.svg)

La hauteur des plans _near_ et _far_ est déterminée
par le champ de vision. La largeur de ces plans est déterminée par le champ de vision et le ratio.

Tout ce qui est dans le _frustum_ est dessiné. Ce qui est à l'extérieur ne l'est pas.
...
Notre _frustum_ démarre à 0.1 unités de la caméra et s'étend jusqu'à 5 unités devant elle.
...
Ensuite, nous créons une [`Scene`](/api/en/scenes/Scene). Dans Three.js, une [`Scene`](/api/en/scenes/Scene) est la racine
...
Ensuite nous créons une géométrie de type [`BoxGeometry`](/api/en/geometries/BoxGeometry) qui contient les données pour un parallélépipède.
...
Nous créons ensuite un maillage ([`Mesh`](/api/en/objects/Mesh)). Dans Three.js, il représente la combinaison
d'une `Geometry` (forme de l'objet) et d'un matériau ([`Material`](/api/en/materials/Material) - aspect
...
Nous pouvons, maintenant, effectuer le rendu de la scène en appelant la fonction _render_ du _renderer_
...
Nous avons également besoin de changer le matériau car [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) ne s'applique pas aux
lumières. Nous le remplaçons par un [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) qui est affecté par les sources lumineuses.
...
et positions en X, puis nous conservons ces instances de [`Mesh`](/api/en/objects/Mesh) dans un tableau.
...
sont partiellement en dehors du _frustum_. Ils sont, de plus,
...
Comme nous pouvons le constater, nous avons 3 objets de type [`Mesh`](/api/en/objects/Mesh), chacun référençant
la même [`BoxGeometry`](/api/en/geometries/BoxGeometry). Chaque [`Mesh`](/api/en/objects/Mesh) référence également un [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) unique
...
Depuis la version 106 (r106), l'utilisation de Three.js privilégie les [modules es6](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import).

Ils sont chargés via le mot-clé `import` dans un script ou en ligne
...
`./` ou par `../`,
ce qui est différent des autres balises telles que `<img>` et `<a>`.
...
des exemples tels que [`OrbitControls.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/controls/OrbitControls.js)
...
Cela est valable aussi lors de l'utilisation d'un CDN. Assurez vous que vos chemins versThis includes when using a CDN. Be `three.modules.js` terminent par
`/build/three.modules.js`. Par exemple :
