...
<li>vous savez ce que c'est qu'un _DOM_, comment écrire du HTML, ainsi que créer des éléments _DOM_
en Javascript ;</li>
<li>vous savez exploiter les
[modules es6](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import)
que ce soit via le mot clef import ou via les balises `<script type="module">` ;</li>
<li>vous avez des connaissances en CSS et savez ce que sont
[les sélecteurs CSS](https://developer.mozilla.org/en-US/docs/Learn/CSS/Introduction_to_CSS/Selectors).</li>
<li>vous connaissez ES5, ES6, voire ES7 ;</li>
<li>vous savez que le navigateur n'exécute que du Javascript de façon événementiel via des fonctions de rappel (_callbacks_) ;</li>
<li>vous savez ce qu'est une fonction de clôture (_closure_).</li>
</ul>
...
## Connaître le fonctionnement des clôtures (_closures_)
...
Cette fonction se _clôt_ sur la variable `foo`. Voici [davantage d'information](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures).
...
`this` prendra la valeur `null` (dans le cas du mode _strict_ ou d'un module) tandis que lorsque vous
...
Ce fonctionnement peut dérouter lorsqu'il est combiné avec les fonctions de rappel (_callbacks_).
...
Il n'y a <strong>PLUS AUCUNE</strong> raison d'utiliser `var`. L'utiliser est dorénavant considéré
...
été ajoutées aux tableaux (_arrays_) et sont utilisés de manière assez intensives en JavaScript moderne.

### Utiliser l'affectation par décomposition (_destructuring_)
...
### Comprendre les accesseurs (_getters_ et _setters_)
...
### Utilisez les fonctions fléchées (_arrow functions_) quand c'est approprié

Cela est particulièrement utile avec les fonctions de rappel (_callbacks_) et les promesses (_promises_).
...
Les promesses (_promises_) aident à l'utilisation de code asynchrone.
...
### Utilisez les gabarits de libellés (_template Literals_)

Les gabarits de libellés sont des chaînes de caractères délimitées par
des accents graves au lieu d'apostrophes doubles (_quotes_).
...
les noms de fonctions et de méthodes sont toutes en _lowerCasedCamelCase_ (c'est
...
Les constructeurs, les noms des classes sont en _CapitalizedCamelCase_ (
...
<div class="threejs_center">
  ![image](/resources/images/vscode-eslint-not-defined.png)
</div>
...
Vous aurez des avertissements (_warnings_) en utilisant `THREE` donc ajoutez `/* global THREE */`
en haut de vos fichiers JavaScript pour notifier à eslint que `THREE` existe.

<div class="threejs_center">
  ![image](/resources/images/vscode-eslint-not-a-constructor.png)
</div>
...
<div class="threejs_center">
  ![image](/resources/images/vscode-eslint-var.png)
</div>
...
<div class="threejs_center">
  ![image](/resources/images/vscode-eslint-let.png)
</div>
