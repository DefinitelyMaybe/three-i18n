...
à n'importe quelle situation. Rendre une page web adaptable (_responsive_)
...
aux _smart-phones_, en passant par les tablettes.
...
En HTML, la balise _body_ a une marge fixée à 5 pixels par défaut donc
la changer à 0 la retire. Modifier la hauteur de _html_ et _body_ à 100%
leur fait occuper toute la fenêtre. Sinon, ils sont seulement aussi large
que le contenu qu'ils contiennent.
...
<iframe src="/editor?url=/manual/examples/responsive-no-resize.html"></iframe>
...
![image](/resources/images/resize-incorrect-aspect.png)
...
![image](/resources/images/resize-low-res.png)
...
<iframe src="/editor?url=/manual/examples/responsive-update-camera.html"></iframe>
...
![image](/resources/images/resize-correct-aspect.png)

Maintenant résolvons le problème de la pixellisation.

Les éléments de type _canvas_ ont deux tailles. La première
...
de dessin (_drawingbuffer_). Dans Three.js, nous pouvons ajuster la taille
...
Remarquez que notre fonction renvoie _true_ si le canvas a été redimensionné.
...
<iframe src="/editor?url=/manual/examples/responsive.html"></iframe>
...
<iframe src="/editor?url=/manual/examples/responsive-paragraph.html&amp;startPane=html"></iframe>

et voici notre même code utilisé dans un éditeur où la zone de contrôle à droite peut être redimensionnée.

<iframe src="/editor?url=/manual/examples/responsive-editor.html&amp;startPane=html"></iframe>
...
HD-DPI est l'acronyme pour _high-density dot per inch_,
...
(_Graphics Processing Units_, les processeurs dédiés de carte graphique).
...
voulez sinon vous risquez d'avoir un taux de rafraîchissement faible (_framerate_).
...
<strong>Ceci est fortement DÉCONSEILLÉ</strong>. Voir ci-dessous.
...
post-processing, ou si nous faisons un _shader_ qui accède à `gl_FragCoord`,
...
<iframe src="/editor?url=/manual/examples/responsive-hd-dpi.html"></iframe>
