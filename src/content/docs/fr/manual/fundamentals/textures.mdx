...
L'[article précédent](setup.html) concerne la configuration nécessaire pour cet article.
...
<ul>
  <li>[Hello Texture](/fr/manual/textures#hello)</li>
  <li>[6 textures, une pour chaque face d'un cube](/fr/manual/textures#six)</li>
  <li>[Téléchargement de textures](/fr/manual/textures#loading)</li>
  <ul>
    <li>[La façon la plus simple](/fr/manual/textures#easy)</li>
    <li>[En attente du chargement d'une texture](/fr/manual/textures#wait1)</li>
    <li>
      [En attente du téléchargement de plusieurs
      textures](/fr/manual/textures#waitmany)
    </li>
    <li>
      [Chargement de textures d'autres origines](/fr/manual/textures#cors)
    </li>
  </ul>
  <li>[Utilisation de la mémoire](/fr/manual/textures#memory)</li>
  <li>[JPG ou PNG](/fr/manual/textures#format)</li>
  <li>[Filtrage et mips](/fr/manual/textures#filtering-and-mips)</li>
  <li>
    [Répétition, décalage, rotation,
    emballage](/fr/manual/textures#uvmanipulation)
  </li>
</ul>

## <a name="hello"></a> Hello Texture

Les textures sont _généralement_ des images qui sont le plus souvent créées dans un programme tiers comme Photoshop ou GIMP. Par exemple, mettons cette image sur un cube.
...
Modifions l'un de nos premiers échantillons. Tout ce que nous avons à faire, c'est de créer un [`TextureLoader`](/api/en/loaders/TextureLoader). Appelons-le avec sa méthode
[`load`](/api/en/loaders/TextureLoader#load) et l'URL d'une image puis définissons le résultat sur la propriété `map` du matériau au lieu de définir `color`.
...
Notez que nous utilisons un [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial), donc pas besoin de lumières.
...
Fabriquons juste 6 matériaux et passons-les sous forme de tableau lors de la création du [`Mesh`](/api/en/objects/Mesh)
...
Il convient de noter, cependant, que tous les types de géométries ne peuvent supporter la prise en charge de plusieurs matériaux. [`BoxGeometry`](/api/en/geometries/BoxGeometry) ne peut utiliser que 6 matériaux, un pour chaque face.
[`ConeGeometry`](/api/en/geometries/ConeGeometry), seulement 2, un pour la base et un pour le cône.
[`CylinderGeometry`](/api/en/geometries/CylinderGeometry) peut recevoir 3 matériaux pour le bas, le haut et le côté.
...
La plupart du code de ce site utilise la méthode la plus simple pour charger les textures. Nous créons un [`TextureLoader`](/api/en/loaders/TextureLoader) puis appelons sa méthode de [`chargement`](/api/en/loaders/TextureLoader#load).
Cela renvoie un objet [`Texture`](/api/en/textures/Texture).
...
### <a name="waitmany"></a>Attendre le chargement de plusieurs textures

Pour attendre que toutes les textures soient chargées, vous pouvez utiliser un [`LoadingManager`](/api/en/loaders/managers/LoadingManager). Créez-en un et transmettez-le à [`TextureLoader`](/api/en/loaders/TextureLoader), puis définissez sa propriété [`onLoad`](/api/en/loaders/managers/LoadingManager#onLoad) avec une fonction de rappel.
...
Le [`LoadingManager`](/api/en/loaders/managers/LoadingManager) a également une propriété [`onProgress`](/api/en/loaders/managers/LoadingManager#onProgress) que nous pouvons définir sur une autre 'callback' pour afficher un indicateur de progression.
...
Les textures sont souvent la partie d'une application Three.js qui utilise le plus de mémoire. Il est important de comprendre qu'en _général_, les textures prennent `width * height * 4 * 1.33` octets de mémoire.
...
Cette image ne pèse que 157ko, elle sera donc téléchargée relativement vite mais [sa taille est en réalité de 3024 x 3761 pixels](resources/images/compressed-but-large-wood-texture.jpg).
En suivant l'équation ci-dessus

```javascript
3024 * 3761 * 4 * 1.33 = 60505764.5
```

Cette image prendra <strong>60 MEGA de MÉMOIRE !</strong> dans Three.js.
Encore quelques textures comme celle-là et vous serez à court de mémoire.

J'en parle car il est important de savoir que l'utilisation de textures a un coût caché. Pour que Three.js utilise la texture, il doit la transmettre au GPU et le GPU _en général_ nécessite que les données de texture soient décompressées.
...
Pour définir le filtre lorsque la texture est dessinée plus grande que sa taille d'origine, définissez la propriété [`texture.magFilter`](/api/en/textures/Texture#magFilter) sur `THREE.NearestFilter` ou
...
Pour définir le filtre lorsque la texture est dessinée plus petite que sa taille d'origine, définissez la propriété [`texture.minFilter`](/api/en/textures/Texture#minFilter) sur l'une des 6 valeurs :
...
Par défaut, les textures dans three.js ne se répètent pas. Pour définir si une texture se répète ou non, il existe 2 propriétés, [`wrapS`](/api/en/textures/Texture#wrapS) pour un habillage horizontal et [`wrapT`](/api/en/textures/Texture#wrapT) pour un habillage vertical.
...
```js
const xOffset = 0.5; // décalage de la moitié de la texture
const yOffset = 0.25; // décalage d'un quart
...
La dernière chose à noter à propos de l'exemple est que si vous modifiez `wrapS` ou `wrapT` sur la texture, vous devez également définir [`texture.needsUpdate`](/api/en/textures/Texture#needsUpdate)
afin que three.js sache qu'il faut mettre à jour ces paramètres. Les autres paramètres sont automatiquement appliqués.
