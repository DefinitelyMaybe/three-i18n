...
creating [`Fog`](/api/en/scenes/Fog) or [`FogExp2`](/api/en/scenes/FogExp2) object and setting it on the scene's
[`fog`](/api/en/scenes/Scene#fog) property.

[`Fog`](/api/en/scenes/Fog) lets you choose `near` and `far` settings which are distances
...
There's also [`FogExp2`](/api/en/scenes/FogExp2) which grows exponentially with distance from the camera.
...
or for [`FogExp2`](/api/en/scenes/FogExp2) it would be
...
[`FogExp2`](/api/en/scenes/FogExp2) is closer to reality but [`Fog`](/api/en/scenes/Fog) is used
...
It's important to note that the fog is applied to _things that are rendered_.
...
[`scene.background`](/api/en/scenes/Scene#background)
property. To pick a background color you attach a [`THREE.Color`](/api/en/math/Color) to it. For example
...
The `.listen()` at the end of the last 2 lines tells lil-gui to _listen_
...
color in sync. So, let's add another _virtual_ property to our helper
...
lil-gui is only manipulating a single value. Fortunately [`THREE.Color`](/api/en/math/Color)
as a [`getHexString`](/api/en/math/Color#getHexString) method
...
One last thing, there is a boolean [`fog`](/api/en/materials/Material#fog)
