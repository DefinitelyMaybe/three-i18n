...
We make a material that is blue but we gave it a small amount of _emissive_ blue
...
Here we made an [`Object3D`](/api/en/core/Object3D). Like a [`Mesh`](/api/en/objects/Mesh) it is also a node in the scene graph
but unlike a [`Mesh`](/api/en/objects/Mesh) it has no material or geometry. It just represents a local space.
...
Again we added more invisible scene graph nodes. The first, an [`Object3D`](/api/en/core/Object3D) called `earthOrbit`
...
One is called an [`AxesHelper`](/api/en/helpers/AxesHelper). It draws 3 lines representing the local

<span style="color:red">X</span>,<span style="color:green">Y</span>, and
<span style="color:blue">Z</span> axes. Let's add one to every node we created.
...
<span style="color:blue">z (blue)</span> axes. Since we are looking straight
down and each of our objects is only rotating around its y axis we don't see
much of the <span style="color:green">y (green)</span> axes.
...
While we're at it let's also add another helper called the [`GridHelper`](/api/en/helpers/GridHelper). It
...
We want to make both a [`GridHelper`](/api/en/helpers/GridHelper) and an [`AxesHelper`](/api/en/helpers/AxesHelper) for each node. We need
...
the visible property of both the [`AxesHelper`](/api/en/helpers/AxesHelper) and [`GridHelper`](/api/en/helpers/GridHelper) for a node.
...
One thing to notice is we set the `renderOrder` of the [`AxesHelper`](/api/en/helpers/AxesHelper)
to 2 and for the [`GridHelper`](/api/en/helpers/GridHelper) to 1 so that the axes get drawn after the grid.
...
earth is in the _local space_ of the `solarSystem`. Similarly if you
turn on the `earthOrbit` you'll see how the moon is exactly 2 units
from the center of the _local space_ of the `earthOrbit`.
...
Here's the scene graph. The meshes are colored in green, the [`Object3D`](/api/en/core/Object3D)s in blue,
...
([`Object3D`](/api/en/core/Object3D)) which just rotates similar to the `earthOrbit` above. A
`targetElevation` ([`Object3D`](/api/en/core/Object3D)) which is a child of the `targetOrbit` provides an
offset from the `targetOrbit` and a base elevation. Childed to that is another
[`Object3D`](/api/en/core/Object3D) called `targetBob` which just bobs up and down relative to the
...
For the tank there's an [`Object3D`](/api/en/core/Object3D) called `tank` which is used to move everything
below it around. The code uses a [`SplineCurve`](/api/en/extras/curves/SplineCurve) which it can ask for positions
...
direction using [`Object3D.lookAt`](/api/en/core/Object3D.lookAt).
...
and then again use [`Object3D.lookAt`](/api/en/core/Object3D.lookAt)
...
Making [`Object3D`](/api/en/core/Object3D) nodes and parenting things to them is an important step to using
