...
Rendering to a render target is almost exactly the same as normal rendering. First we create a [`WebGLRenderTarget`](/api/en/renderers/WebGLRenderTarget).
...
Then we need a [`Camera`](/api/en/cameras/Camera) and a [`Scene`](/api/en/scenes/Scene)
...
The [`Scene`](/api/en/scenes/Scene) and [`Camera`](/api/en/cameras/Camera) from the previous article are still there. We'll use them to render to the canvas.
...
A few notes about using [`WebGLRenderTarget`](/api/en/renderers/WebGLRenderTarget).

<ul>
<li>
By default [`WebGLRenderTarget`](/api/en/renderers/WebGLRenderTarget) creates 2 textures. A color texture and a depth/stencil texture. If you don't need the depth or stencil textures you can request to not create them by passing in options. Example:
