...
the [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera). It gives a 3d view where things in the distance appear
smaller than things up close.

The [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera) defines a _frustum_. [A _frustum_ is a solid pyramid shape with
...
_frustum_ and my eyes would glaze over. Understanding it's the name of a type of solid
shape made those descriptions suddenly make more sense 😅

A [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera) defines its frustum based on 4 properties. `near` defines where the
...
[`updateProjectionMatrix`](/api/en/cameras/PerspectiveCamera#updateProjectionMatrix) function
...
help us with events so both cameras can easily have their own [`OrbitControls`](/examples/controls/OrbitControls).
...
Then in our code we'll add a [`CameraHelper`](/api/en/helpers/CameraHelper). A [`CameraHelper`](/api/en/helpers/CameraHelper) draws the frustum for a [`Camera`](/api/en/cameras/Camera)
...
And we'll set our existing [`OrbitControls`](/examples/controls/OrbitControls) to respond to the first
view element only.
...
Let's make a second [`PerspectiveCamera`](/api/en/cameras/PerspectiveCamera) and a second [`OrbitControls`](/examples/controls/OrbitControls).
The second [`OrbitControls`](/examples/controls/OrbitControls) is tied to the second camera and gets input
...
This is an example of _z fighting_ where the GPU on your computer does not have
...
`logarithmicDepthBuffer` when we create the [`WebGLRenderer`](/api/en/renderers/WebGLRenderer)
...
The 2nd most common camera is the [`OrthographicCamera`](/api/en/cameras/OrthographicCamera). Rather than
...
Let's change the 2 view example above to use an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera)
in the first view.

First let's setup an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera).
...
the [`OrbitControls`](/examples/controls/OrbitControls) can also control zoom. For example the scroll wheel on
a mouse will zoom via the [`OrbitControls`](/examples/controls/OrbitControls).

Last we just need to change the part that renders the left
side to update the [`OrthographicCamera`](/api/en/cameras/OrthographicCamera).
...
and now you can see an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) at work.

<iframe src="/editor?url=/manual/examples/cameras-orthographic-2-scenes.html"></iframe>

An [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) is most often used if using three.js
...
We'll parent each plane to a [`THREE.Object3D`](/api/en/core/Object3D) to make it easy to offset
...
`planes` is an array of [`THREE.Mesh`](/api/en/objects/Mesh), one for each plane.
...
Another common use for an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera) is to draw the
