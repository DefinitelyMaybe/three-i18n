...
[`BufferGeometry`](/api/en/core/BufferGeometry) is three.js's way of representing all geometry. A [`BufferGeometry`](/api/en/core/BufferGeometry)
essentially a collection _named_ of [`BufferAttribute`](/api/en/core/BufferAttribute)s.
Each [`BufferAttribute`](/api/en/core/BufferAttribute) represents an array of one type of data: positions,
normals, colors, uv, etc... Together, the named [`BufferAttribute`](/api/en/core/BufferAttribute)s represent
_parallel arrays_ of all the data for each vertex.
...
They represent _parallel arrays_ which means that the Nth set of data in each
...
A single _vertex_ is the combination of all of its parts. If a vertex needs any
part to be different then it must be a different vertex.

As a simple example let's make a cube using [`BufferGeometry`](/api/en/core/BufferGeometry). A cube is interesting
...
[`BufferAttribute`](/api/en/core/BufferAttribute)s and add them to a [`BufferGeometry`](/api/en/core/BufferGeometry).
...
Finally we can create a [`BufferGeometry`](/api/en/core/BufferGeometry) and then a [`BufferAttribute`](/api/en/core/BufferAttribute) for each array
and add it to the [`BufferGeometry`](/api/en/core/BufferGeometry).
...
of type `Float32Array`. A [`BufferAttribute`](/api/en/core/BufferAttribute) requires a TypedArray not a native
array. A [`BufferAttribute`](/api/en/core/BufferAttribute) also requires you to tell it how many components there
...
for the 36 vertices we need drawn to make 12 triangles by calling [`BufferGeometry.setIndex`](/api/en/core/BufferGeometry.setIndex) with an array of indices.
...
[`BufferGeometry`](/api/en/core/BufferGeometry) has a [`computeVertexNormals`](/api/en/core/BufferGeometry#computeVertexNormals) method for computing normals if you
...
Because I'm lazy I used a small hierarchy of 3 [`Object3D`](/api/en/core/Object3D) objects to compute
...
I hope these were useful examples of how to use [`BufferGeometry`](/api/en/core/BufferGeometry) directly to
make your own geometry and how to dynamically update the contents of a
[`BufferAttribute`](/api/en/core/BufferAttribute).

[](resources/threejs-geometry.svg)
