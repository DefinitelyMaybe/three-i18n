---
title: Using OffscreenCanvas in a Web Worker
sidebar:
  order: 2
---
...
Getting _started_ using it is pretty straight forward. Let's port the 3 spinning cube
...
of objects that are part of the first object that we want _transferred_
to the worker. These objects will not be cloned. Instead they will be _transferred_
...
Let's take it one more step and add in the [`OrbitControls`](/examples/controls/OrbitControls).
This will be little more involved. The [`OrbitControls`](/examples/controls/OrbitControls) use
...
It inherits from [`EventDispatcher`](/api/en/core/EventDispatcher) which provides methods like
...
In our shared three.js code we need to import the [`OrbitControls`](/examples/controls/OrbitControls) and set them up.
...
that the [`OrbitControls`](/examples/controls/OrbitControls) need a few more things.
...
This will give the [`OrbitControls`](/examples/controls/OrbitControls) something to inspect which
...
[`OrbitControls`](/examples/controls/OrbitControls) and with our own code.
