---
title: Use three.js as Background in HTML
sidebar:
  order: 7
---
...
you create the [`WebGLRenderer`](/api/en/renderers/WebGLRenderer). This prevents the browser from
...
canvas to be transparent pass in [`alpha:true`](/api/en/renderers/WebGLRenderer#alpha) when you create
the [`WebGLRenderer`](/api/en/renderers/WebGLRenderer)
...
You probably also want to tell it that your results are <strong>not</strong> using premultiplied alpha
...
[`premultipliedAlpha: true`](/api/en/renderers/WebGLRenderer#premultipliedAlpha) but defaults
to materials outputting [`premultipliedAlpha: false`](/api/en/materials/Material#premultipliedAlpha).
...
<ul>
  <li>Set the canvas CSS `position` to `fixed` as in</li>
</ul>
