...
If you look at the [primitives article](primitives.html) you'll see [`TextGeometry`](/api/en/geometries/TextGeometry) which
...
We'll add some [`OrbitControls`](/examples/controls/OrbitControls) like we did in [the article on lighting](lights.html).
...
This _kind of_ works because the normalized coordinates we computed include a `z`
...
<div class="threejs_center">
  ![image](/resources/images/label-sorting-issue.png)
</div>
...
```js
{
  ...
  const texture = loader.load(
    "resources/data/world/country-outlines-4k.png",
    render,
  );
...
position on the globe and take the _dot product_. The dot product returns the cosine
