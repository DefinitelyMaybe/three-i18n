...
make a [`Renderer`](/api/en/constants/Renderer).
...
The solution is one canvas that fills the viewport in the background and some other element to represent each "virtual" canvas. We make a single [`Renderer`](/api/en/constants/Renderer) and then one [`Scene`](/api/en/scenes/Scene) for each virtual canvas. We'll then check the positions of the virtual canvas elements and if they are on the screen we'll tell THREE.js to draw their scene at the correct place.
...
to only render to part of the canvas by turning on the _scissor_
test with [`Renderer.setScissorTest`](/api/en/constants/Renderer.setScissorTest) and then setting both the scissor and the viewport with [`Renderer.setViewport`](/api/en/constants/Renderer.setViewport) and [`Renderer.setScissor`](/api/en/constants/Renderer.setScissor).
...
And if we [quickly scroll up and down](../examples/multiple-scenes-v2.html) we'll see the issue. Here's a animation of scrolling slowed down by 10x.
...
We'll change the scene setup code to just be a map of names to _scene initialization functions_ that return a _scene render function_.
