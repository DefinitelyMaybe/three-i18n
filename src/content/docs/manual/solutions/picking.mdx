...
_Picking_ refers to the process of figuring out which object a user clicked on or touched. There are tons of ways to implement picking each with their tradeoffs. Let's go over the 2 most common.

Probably the most common way of _picking_ is by doing raycasting which means to _cast_ a ray from the mouse through the frustum of the scene and computing which objects that ray intersects. Conceptually it's very simple.
...
Of course we could call this function only when the user pressed the mouse _down_ which is probably usually what you want but for this example we'll pick every frame whatever is under the mouse. To do this we first need to track where the mouse
is
...
<iframe src="/editor?url=/manual/examples/picking-raycaster.html"></iframe>
...
<div class="threejs_center">![image](/resources/images/frame.png)</div>
...
<iframe src="/editor?url=/manual/examples/picking-raycaster-transparency.html"></iframe>

Try to pick something through a box and you can't

<div class="threejs_center">
  ![image](/resources/images/picking-transparent-issue.jpg)
</div>
...
Note that we are abusing the [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) here. By setting its `emissive` to our id and the `color` and `specular` to 0 that will end up rendering the id only where the texture's alpha is greater than `alphaTest`. We also need to set `blending` to `NoBlending` so that the id is not multiplied by alpha.

Note that abusing the [`MeshPhongMaterial`](/api/en/materials/MeshPhongMaterial) might not be the best solution as it will still calculate all our lights when drawing the picking scene even though we don't need those calculations. A more optimized solution would make a custom shader that just writes the id where the texture's alpha is greater than `alphaTest`.
...
Then let's change the `PickHelper` into a `GPUPickHelper`. It will use a [`WebGLRenderTarget`](/api/en/renderers/WebGLRenderTarget) like we covered the [article on render targets](rendertargets.html). Our render target here is only a single pixel in size, 1x1.
...
<iframe src="/editor?url=/manual/examples/picking-gpu.html"></iframe>
