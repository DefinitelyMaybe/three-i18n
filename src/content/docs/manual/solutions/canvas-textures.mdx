...
at runtime. One way to do this is to use a [`CanvasTexture`](/api/en/textures/CanvasTexture).
...
our canvas by creating a [`CanvasTexture`](/api/en/textures/CanvasTexture) and passing it the canvas we created.
...
of the [`CanvasTexture`](/api/en/textures/CanvasTexture) to tell three.js to update the texture with
...
[`Object3D`](/api/en/core/Object3D) and adjust their positions. This would let us move the
...
What's left is to add some [`OrbitControls`](/examples/controls/OrbitControls) so we can move
the camera.
...
The labels in this article are _inside the 3D world_ which is good if you want them
