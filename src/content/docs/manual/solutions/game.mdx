...
I ended up writing quite a bit of code to make this simple _unfinished_
...
Fortunately three.js provides the [`LoadingManager`](/api/en/loaders/managers/LoadingManager) just for this purpose.
We create a [`LoadingManager`](/api/en/loaders/managers/LoadingManager) and pass it to the other loaders. The
[`LoadingManager`](/api/en/loaders/managers/LoadingManager) provides both [`onProgress`](/api/en/loaders/managers/LoadingManager#onProgress) and
[`onLoad`](/api/en/loaders/managers/LoadingManager#onLoad) properties we can attach callbacks to.
The [`onLoad`](/api/en/loaders/managers/LoadingManager#onLoad) callback will be called when
all files have been loaded. The [`onProgress`](/api/en/loaders/managers/LoadingManager#onProgress) callback
...
This code will load all the models above and the [`LoadingManager`](/api/en/loaders/managers/LoadingManager) will call
`init` when done. We'll use the `models` object later to let us access the
loaded models so the [`GLTFLoader`](/examples/loaders/GLTFLoader) callback for each individual model attaches
...
So, let's add an [`onProgress`](/api/en/loaders/managers/LoadingManager#onProgress) callback. It will be
...
to a new [`Object3D`](/api/en/core/Object3D). We need to parent it to another object because when
...
To play the animations each model we clone needs an [`AnimationMixer`](/api/en/animation/AnimationMixer).
An [`AnimationMixer`](/api/en/animation/AnimationMixer) contains 1 or more [`AnimationAction`](/api/en/animation/AnimationAction)s. An
[`AnimationAction`](/api/en/animation/AnimationAction) references an [`AnimationClip`](/api/en/animation/AnimationClip). [`AnimationAction`](/api/en/animation/AnimationAction)s
...
[`AnimationClip`](/api/en/animation/AnimationClip) and create an action for it. The default is for
...
We called [`play`](/api/en/animation/AnimationAction#play) to start the action and stored
off all the `AnimationMixers` in an array called `mixers`. Finally
we need to update each [`AnimationMixer`](/api/en/animation/AnimationMixer) in our render loop by computing
the time since the last frame and passing that to [`AnimationMixer.update`](/api/en/animation/AnimationMixer.update).
...
The code above makes an array of [`AnimationAction`](/api/en/animation/AnimationAction)s,
one for each [`AnimationClip`](/api/en/animation/AnimationClip). It makes an array of objects, `mixerInfos`,
with references to the [`AnimationMixer`](/api/en/animation/AnimationMixer) and all the [`AnimationAction`](/api/en/animation/AnimationAction)s
...
ton more manipulation of [`AnimationAction`](/api/en/animation/AnimationAction) objects.
...
In an Entity Component System an object in a game is called an _entity_ that consists
of a bunch of _components_. You build up entities by deciding which components to
...
of components and a three.js [`Object3D`](/api/en/core/Object3D).
...
then sets up an [`AnimationMixer`](/api/en/animation/AnimationMixer). `setAnimation` adds a [`AnimationAction`](/api/en/animation/AnimationAction) for a
particular [`AnimationClip`](/api/en/animation/AnimationClip) if one does not already exist and disables all
existing actions.
...
The code above uses [`Object3D.transformOnAxis`](/api/en/core/Object3D.transformOnAxis) to move the player
forward. [`Object3D.transformOnAxis`](/api/en/core/Object3D.transformOnAxis) works in local space so it only
works if the object in question is at the root of the scene, not if it's
parented to something else [1](/en/manual/game#parented)
...
We can do that by using the three.js [`Frustum`](/api/en/math/Frustum) class to check if a point
...
The code above sets the [`AnimationMixer.timeScale`](/api/en/animation/AnimationMixer.timeScale) to set the playback
...
I uses a [`PolarGridHelper`](/api/en/helpers/PolarGridHelper) to draw a circle around each character
...
know which objects _might_ be colliding then you need to do more work to check if
they are _actually_ colliding. Often even checking the bounding spheres is too
...
One more thing, many game systems have something called [_coroutines_](https://www.google.com/search?q=coroutines).
...
It uses [`SpriteMaterial`](/api/en/materials/SpriteMaterial) and a [`Sprite`](/api/en/objects/Sprite) like we covered in
...
All it does is setup a [`Sprite`](/api/en/objects/Sprite), then pick a random velocity and move
the transform at that velocity for 60 frames while fading out the note
by setting the material's [`opacity`](/api/en/materials/Material#opacity).
...
where as with coroutines you can often just _fire and forget_.
...
Maybe I should promote a game jam. If you click the _jsfiddle_ or _codepen_ buttons
...
<div class="footnotes">
  [<a id="parented">1</a>]: technically it would still work if none of the
  parents have any translation, rotation, or scale
  [§](/en/manual/game#parented-backref).
</div>
