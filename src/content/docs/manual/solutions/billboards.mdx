...
In [a previous article](canvas-textures.html) we used a [`CanvasTexture`](/api/en/textures/CanvasTexture)
to make labels / badges on characters. Sometimes we'd like to make labels or
other things that always face the camera. Three.js provides the [`Sprite`](/api/en/objects/Sprite) and
[`SpriteMaterial`](/api/en/materials/SpriteMaterial) to make this happen.

Let's change the badge example from [the article on canvas textures](canvas-textures.html)
to use [`Sprite`](/api/en/objects/Sprite) and [`SpriteMaterial`](/api/en/materials/SpriteMaterial)
...
Then we'll make a function that makes a [`Mesh`](/api/en/objects/Mesh) each for the trunk and top
of a tree and parents both to an [`Object3D`](/api/en/core/Object3D).
...
waste space as well so instead we just _fudge_ it.
