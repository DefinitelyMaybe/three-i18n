...
<div class="threejs_center">![image](/resources/images/windmill-obj.jpg)</div>

I downloaded the .blend file from that site, loaded it into [Blender](https://blender.org)
and exported it as an .OBJ file.

<div class="threejs_center">
  ![image](/resources/images/windmill-export-as-obj.jpg)
</div>
...
<div class="threejs_center">
  ![image](/resources/images/windmill-export-options.jpg)
</div>
...
[`HemisphereLight`](/api/en/lights/HemisphereLight) and one [`DirectionalLight`](/api/en/lights/DirectionalLight). I also removed all the GUI stuff
related to adjusting the lights. I also removed the cube and sphere
that were being added to the scene.

From that the first thing we need to do is include the [`OBJLoader`](/examples/loaders/OBJLoader) loader in our script.

```js
import { OBJLoader } from "three/addons/loaders/OBJLoader.js";
```

Then to load the .OBJ file we create an instance of [`OBJLoader`](/examples/loaders/OBJLoader),
...
<iframe src="/editor?url=/manual/examples/load-obj-no-materials.html"></iframe>
...
<div class="threejs_center">
  ![image](/resources/images/windmill-exported-files.png)
</div>
...
export those files to by picking <strong>File->External Data->Unpack All Into Files</strong>

<div class="threejs_center">
  ![image](/resources/images/windmill-export-textures.jpg)
</div>

and then choosing <strong>Write Files to Current Directory</strong>

<div class="threejs_center">
  ![image](/resources/images/windmill-overwrite.jpg)
</div>

This ends up writing the files in the same folder as the .blend file
in a sub folder called <strong>textures</strong>.

<div class="threejs_center">
  ![image](/resources/images/windmill-exported-texture-files.png)
</div>
...
<div class="threejs_center">
  ![image](/resources/images/windmill-exported-files-with-textures.png)
</div>

Now that we have the textures available we can load the .MTL file.

First we need to include the [`MTLLoader`](/examples/loaders/MTLLoader);
...
the just loaded materials on to the [`OBJLoader`](/examples/loaders/OBJLoader) itself via the `setMaterials`
...
<iframe src="/editor?url=/manual/examples/load-obj-materials.html"></iframe>

Note that if we spin the model around you'll see the windmill cloth
disappears

<div class="threejs_center">
  ![image](/resources/images/windmill-missing-cloth.jpg)
</div>
...
<ol>
<li>
Loop over all the materials after loading them and set them all to double sided.
...
<li>
Manually set a specific material
...
<li>
Realizing that the .MTL file is limited we could just not use it
and instead create materials ourselves.

In this case we'd need to look up the [`Mesh`](/api/en/objects/Mesh) object after
...
<div class="threejs_center">
  ![image](/resources/images/windmill-blocky.jpg)
</div>
...
<div class="threejs_center">
  ![image](/resources/models/windmill/windmill_001_base_NOR.jpg)
</div>
...
<iframe src="/editor?url=/manual/examples/load-obj-materials-fixed.html"></iframe>
...
<div class="threejs_center">![image](/resources/images/windmill-obj-2.jpg)</div>
...
<iframe src="/editor?url=/manual/examples/load-obj-wat.html"></iframe>
...
<div class="threejs_center">
  ![image](/resources/images/camera-inside-windmill.svg)
</div>
...
view the entire scene. Note that there is no _right_ answer
...
<div class="threejs_center">
  ![image](/resources/images/camera-fit-scene.svg)
</div>
...
Using basic _right triangle_ trigonometry and [SOHCAHTOA](https://www.google.com/search?q=SOHCAHTOA),
given we know the field of view for the frustum and we know the size of the box we can compute the _distance_.

<div class="threejs_center">
  ![image](/resources/images/field-of-view-camera.svg)
</div>
...
to fit it inside the frustum. We also updated the [`OrbitControls`](/examples/controls/OrbitControls) so the camera will orbit the center
of the scene.

Now if we try that we get...

<iframe src="/editor?url=/manual/examples/load-obj-auto-camera.html"></iframe>
...
<div class="threejs_center">
  ![image](/resources/images/computed-camera-position.svg)
</div>
...
<div class="threejs_center">
  ![image](/resources/images/tiny-ground-plane.jpg)
</div>
...
<iframe src="/editor?url=/manual/examples/load-obj-auto-camera-xz.html"></iframe>
...
find on the net then maybe, just maybe, you might want to load TGA files. ([\*](/en/manual/load-obj#loading-scenes))
...
and then set them on the [`OBJLoader`](/examples/loaders/OBJLoader)
...
Issue #1: The three [`MTLLoader`](/examples/loaders/MTLLoader) creates materials that multiply the material's diffuse color by the diffuse texture map.
...
<iframe src="/editor?url=/manual/examples/load-obj-materials-windmill2.html"></iframe>
