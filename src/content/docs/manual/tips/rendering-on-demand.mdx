...
`requestAnimationFrame` loop or "_rAF loop_" something like this
...
We need to render anytime the [`OrbitControls`](/examples/controls/OrbitControls) change the camera settings.
Fortunately the [`OrbitControls`](/examples/controls/OrbitControls) dispatch a `change` event anytime something
changes.
...
The [`OrbitControls`](/examples/controls/OrbitControls) have options to add a kind of inertia to make them feel less
...
so that the [`OrbitControls`](/examples/controls/OrbitControls) can continue to give us new camera settings as they
