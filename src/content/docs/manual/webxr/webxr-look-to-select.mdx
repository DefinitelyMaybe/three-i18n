...
<strong>
  ...
  won't work. See [previous article](webxr.html) as to why
</strong>
...
<iframe src="/editor?url=/manual/examples/webxr-look-to-select.html"></iframe>
...
First we make an [`OrthographicCamera`](/api/en/cameras/OrthographicCamera)
...
Next let's make a 2 color texture. We'll use a [`DataTexture`](/api/en/textures/DataTexture)
...
We'll then use that texture on a [`TorusGeometry`](/api/en/geometries/TorusGeometry)
...
<iframe src="/editor?url=/manual/examples/webxr-look-to-select-selector.html"></iframe>

A few things to notice <strong>and try</strong>.
...
This gives as an _inverse_ type of effect. Comment out
...
<li>
We use a [`TorusGeometry`](/api/en/geometries/TorusGeometry) and not a [`RingGeometry`](/api/en/geometries/RingGeometry)

For whatever reason the [`RingGeometry`](/api/en/geometries/RingGeometry) uses a flat
UV mapping scheme. Because of this if we use a [`RingGeometry`](/api/en/geometries/RingGeometry)
the texture slides horizontally across the ring instead of
around it like it does above.

Try it out, change the [`TorusGeometry`](/api/en/geometries/TorusGeometry) to a [`RingGeometry`](/api/en/geometries/RingGeometry)
(it's just commented out in the example above) and you'll see what I
mean.

The _proper_ thing to do (for some definition of _proper_) would be
to either use the [`RingGeometry`](/api/en/geometries/RingGeometry) but fix the texture coordinates
...
with a [`MeshBasicMaterial`](/api/en/materials/MeshBasicMaterial) it will look exactly like a ring and the
...
so that we can associate each [`Mesh`](/api/en/objects/Mesh) with its partner.
...
And with that we should have a pretty decent _look to select_ implementation.

<iframe src="/editor?url=/manual/examples/webxr-look-to-select-w-cursor.html"></iframe>
