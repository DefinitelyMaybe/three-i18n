...
<strong>
  ...
  device. Without one they won't work. See [this article](webxr.html) as to why
</strong>
...
controllers. Each of the controllers are [`Object3D`](/api/en/core/Object3D) objects which give
...
objects we don't by parenting them under another [`Object3D`](/api/en/core/Object3D).
...
Like before we use a [`Raycaster`](/api/en/core/Raycaster) but this time we take the ray from the controller.
...
To do that we can extend three.js's [`EventDispatcher`](/api/en/core/EventDispatcher) and then we'll check
...
We use the [`Object3D.attach`](/api/en/core/Object3D.attach) to re-parent
