...
<ul>
<li>
먼저 [`Renderer`](/api/ko/constants/Renderer)가 있습니다. Three.js의 핵심 객체이죠. [`Renderer`](/api/ko/constants/Renderer)는
[`Scene`](/api/ko/scenes/Scene)과 [`Camera`](/api/ko/cameras/Camera) 객체를 넘겨 받아 카메라의 [_절두체(frustum)_](https://ko.wikipedia.org/wiki/%EC%A0%88%EB%91%90%EC%B2%B4)
안 3D 씬의 일부를 평면(2차원) 이미지로 렌더링합니다.

</li>
<li>
[씬 그래프(Scene graph)](scenegraph.html)는 [`Scene`](/api/ko/scenes/Scene) 또는
다수의 [`Mesh`](/api/ko/objects/Mesh), [`Light`](/api/ko/lights/Light), [`Group`](/api/ko/objects/Group), [`Object3D`](/api/ko/core/Object3D), [`Camera`](/api/ko/cameras/Camera)로 이루어진
트리 구조와 유사합니다. [`Scene`](/api/ko/scenes/Scene)은 씬 그래프의 최상위 노드로서 배경색(background color),
안개(fog) 등의 요소를 포함합니다. [`Scene`](/api/ko/scenes/Scene)에 포함된 객체들 또한 부모/자식의
...
[`Camera`](/api/ko/cameras/Camera)가 도표에서 반쯤 나간 것이 보이나요? 이는 의도된 것으로, 다른 객체와 달리
[`Camera`](/api/ko/cameras/Camera)는 굳이 씬 그래프에 포함될 필요가 없음을 보여주기 위함입니다. 물론 다른
객체와 마찬가지로 [`Camera`](/api/ko/cameras/Camera) 또한 다른 객체의 자식 객체가 될 수 있습니다. 이러면
부모 객체에 따라 [`Camera`](/api/ko/cameras/Camera) 또한 움직이겠죠. [씬 그래프에 관한 글](scenegraph.html)
마지막에 여러개의 [`Camera`](/api/ko/cameras/Camera)를 넣는 예제가 있으니 참고하시기 바랍니다.

</li>
<li>
[`Mesh`](/api/ko/objects/Mesh)는 어떤 [`Material`](/api/ko/materials/Material)로 하나의 `Geometry`를 그리는 객체입니다.
[`Material`](/api/ko/materials/Material), `Geometry`는 재사용이 가능하여 여러개의 [`Mesh`](/api/ko/objects/Mesh)가 하나의
[`Material`](/api/ko/materials/Material) 또는 `Geometry`를 동시에 참조할 수 있습니다. 파란색 정육면체 2개를
그린다고 해보죠. 일단 두 정육면체의 위치가 달라야 하니, 2개의 [`Mesh`](/api/ko/objects/Mesh)가
필요합니다. 그리고 정점(vertex, 꼭지점) 데이터를 가진 한 개의 `Geometry`와
채색을 위한 하나의 [`Material`](/api/ko/materials/Material)이 필요하겠죠. 이때 각 [`Mesh`](/api/ko/objects/Mesh)는 객체를 복사할
필요 없이, 같은 `Geometry` 그리고 [`Material`](/api/ko/materials/Material)을 참조할 수 있습니다.
...
<li>
[`Material`](/api/ko/materials/Material)은 기하학 객체를 그리는 데 사용하는 [표면 속성](materials.html)입니다.
색이나 밝기 등을 지정할 수 있죠. 하나의 [`Material`](/api/ko/materials/Material)는 여러개의 [`Texture`](/api/ko/textures/Texture)를 사용할 수
있습니다. 기하학 객체의 표면을 이미지로 덮어씌울 때 주로 사용하죠.

</li>
<li>
[`Texture`](/api/ko/textures/Texture)는 이미지나 [파일에서 로드한 이미지](textures.html),
[canvas로 생성한 이미지](canvas-textures.html)
또는 [다른 `Scene` 객체에서 렌더링한 결과물](rendertargets.html)에 해당합니다.

</li>
<li>
[`Light`](/api/ko/lights/Light)는 [여러 종류의 광원에 해당](lights.html)합니다.
...
canvas 요소(element)를 참조한 이후 [`WebGLRenderer`](/api/ko/renderers/WebGLRenderer)를 생성했습니다.
...
이제 [`Scene`](/api/ko/scenes/Scene)을 만듭니다. Three.js에서 [`Scene`](/api/ko/scenes/Scene)이란 씬 그래프에서 가장 상단에
위치한 요소입니다. 뭔가를 화면에 렌더링하고 싶다면 먼저 [`Scene`](/api/ko/scenes/Scene)에 추가해야 하죠.
...
[`BoxGeometry`](/api/ko/geometries/BoxGeometry) 생성자를 호출하겠습니다.
...
다음으로 [`Material`](/api/ko/materials/Material)을 만들고 색을 지정합니다. 색을 지정할 때는 CSS처럼
...
그리고 앞서 만든 `Geometry`(물체의 형태)와 [`Material`](/api/ko/materials/Material)(물체의 색, 밝기, 질감 등)을
이용해 [`Mesh`](/api/ko/objects/Mesh)를 만듭니다. [`Mesh`](/api/ko/objects/Mesh)는 `Geometry`, [`Material`](/api/ko/materials/Material) 외에도 물체의 위치, 방향,
크기 등을 담은 객체입니다.
...
마지막으로 완성된 정육면체 [`Mesh`](/api/ko/objects/Mesh)를 [`Scene`](/api/ko/scenes/Scene)에 넣어줍니다.

```js
scene.add(cube);
```

`renderer`의 `render` 메서드에 [`Scene`](/api/ko/scenes/Scene)과 [`Camera`](/api/ko/cameras/Camera)를 넘겨주면 화면을 렌더링할
수 있습니다.
...
[`DirectionalLight`](/api/ko/lights/DirectionalLight)를 사용해보도록 하죠.
...
`DirectionalLight`에는 `위치(position)`와 `목표(target)` 속성이 있습니다.
기본값은 `0, 0, 0` 이죠. 먼저 `position`을 `-1, 2, 4`로 설정해 카메라보다
약간 동서쪽, Z축으로는 약간 위로 보냅니다. `target`은 기본값 `0, 0, 0` 그대로
두어 공간의 중앙을 비추도록 합니다.

그리고 `material`도 바꿔야 합니다. [`MeshBasicMaterial`](/api/ko/materials/MeshBasicMaterial)은 광원에 반응하지 않으니,
광원에 반응하는 [`MeshPhongMaterial`](/api/ko/materials/MeshPhongMaterial)로 바꿉니다(`phong`은 광원 반사 모델을 처음 개발한 사람 이름. 역주).
...
미리 만들어 놨던 `Geometry`를 사용하고, [`Material`](/api/ko/materials/Material)만 바꿔 다른 색을
가진 큐브 2개를 만들겠습니다.

먼저 함수를 하나 만들어보겠습니다. 이 함수는 넘겨받은 색상값으로
새로운 [`Material`](/api/ko/materials/Material)을 만들고, 넘겨받은 `Geometry`와 조합해 새로운
[`Mesh`](/api/ko/objects/Mesh)를 만듭니다. 그리고 씬에 추가한 후 넘겨받은 X축 값을 통해 물체를 이동시키죠.
...
그림을 보면 각 [`Mesh`](/api/ko/objects/Mesh) 객체는 같은 [`BoxGeometry`](/api/ko/geometries/BoxGeometry)를 참조합니다. 그러나
각 [`Mesh`](/api/ko/objects/Mesh)는 다른 [`MeshPhongMaterial`](/api/ko/materials/MeshPhongMaterial)을 참조하므로 다른 색을 띄죠.
...
Three.js r106 릴리즈 이후에서는 three.js를 [es6 모듈](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Statements/import)로 사용하길 권장합니다.

es6 모듈은 js 파일이나 인라인 `<script type="module">` 태그 안에서
...
경로는 절대경로나 상대경로를 사용해야 하며, 상대경로는 `<img>`, `<a>` 태그와
달리 `./` 또는 `../`로 시작해야 합니다.
...
이유인 즉 [`OrbitControls.js`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/controls/OrbitControls.js)
...
아래는 CDN을 사용하는 예시입니다. `three.modules.js`의 경로가 `/build/three.modules.js`
로 끝나야 한다는 것을 명심하세요.
