...
먼저 뭔가 변화를 일으킬 수 있는 요소가 필요하니 [`OrbitControls`](/examples/controls/OrbitControls)를 추가합니다.
...
이제 [`OrbitControls`](/examples/controls/OrbitControls)가 카메라 설정을 바꿀 때마다 직접 `render` 함수를 호출해야
합니다. 뭔가 복잡할 것 같지만 다행히 [`OrbitControls`](/examples/controls/OrbitControls)에는 `change` 이벤트가 있습니다.
...
[`OrbitControls`](/examples/controls/OrbitControls)에는 관성(inertia) 옵션이 있습니다. `enableDamping` 속성을 ture로
설정하면 동작이 좀 더 부드러워지죠.
...
또한 [`OrbitControls`](/examples/controls/OrbitControls)가 부드러운 동작을 구현할 때 변경된 카메라 값을 계속 넘겨주도록
