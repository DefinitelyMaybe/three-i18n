...
실행이 잘 된 예제에 이 쉐이더를 적용해보면 법선이 _보통(normally)_ 어떻게 생겼는지 알 수 있습니다. 만약 법선이 정상적으로 보이지 않는다면 그게 다음 문제를 찾을 단서가 되겠죠. fragment 쉐이더에서 법선의 값을 변경하는 경우 같은 방법을 이용해 결과값을 렌더링할 수 있습니다.
...
텍스처를 확인하려면 [`CanvasTexture`](/api/ko/textures/CanvasTexture)나 [`DataTexture`](/api/ko/textures/DataTexture)를 사용해보기 바랍니다.
