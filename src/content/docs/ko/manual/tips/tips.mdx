---
title: Three.js를 HTML 요소의 배경으로 사용하기
sidebar:
  order: 7
---
...
브라우저에서 스크린샷을 찍을 수 있는 방법은 2가지 정도가 있습니다. 예전부터 사용하던 [`canvas.toDataURL`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLCanvasElement/toDataURL)과, 새로 등장한 [`canvas.toBlob`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLCanvasElement/toBlob)이 있죠.
...
움직이는 물체로 사용자가 그림을 그리게 한다고 해봅시다. 이걸 구현하려면 [`WebGLRenderer`](/api/ko/renderers/WebGLRenderer)를 생성할 때 `preserveDrawingBuffer: true`를 설정해야 합니다. 또한 Three.js가 캔버스를 초기화하지 않도록 해주어야 하죠.
...
아무런 설정을 하지 않는다면 Three.js는 기본적으로 캔버스를 불투명하게 렌더링합니다. 캔버스를 투명하게 만들려면 [`WebGLRenderer`](/api/ko/renderers/WebGLRenderer)를 생성할 때 [`alpha: true`](/api/ko/renderers/WebGLRenderer#alpha)를 넘겨줘야 하죠.
...
또한 캔버스가 premultiplied 알파(미리 계산된 alpha 값, straight alpha 또는 associated alpha라고도 불림)를 사용하지 <strong>않도록</strong> 하게끔 하려면 아래처럼 값을 설정해줘야 합니다.
...
Three.js는 기본적으로 캔버스에는 [`premultipliedAlpha: true`](/api/ko/renderers/WebGLRenderer#premultipliedAlpha)를 사용하지만 재질(material)에는 [`premultipliedAlpha: false`](/api/ko/materials/Material#premultipliedAlpha)를 사용합니다.
...
<ul>
  <li>캔버스 요소의 CSS `position`을 `fixed`로 설정한다.</li>
</ul>
