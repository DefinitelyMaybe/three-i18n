...
<strong>
  NOTE: 이 페이지의 예시에는 VR 지원 기기가 필요합니다. VR 기기 없이는 동작하지
  않으며 그 이유를 [이전 글](webxr.html) 에서 확인할 수 있습니다.
</strong>

[이전 글](webxr.html)에서 우리는 three.js를 사용한 매우 간단한 VR 예제를 살펴보고 다양한 종류의 VR 시스템에 대해 이야기했습니다.
...
"Look to Select"를 구현해봅시다! 먼저 [이전 글의 예시](webxr.html)에서 시작해 [Three.js 피킹](picking.html)에서 만든 `PickHelper`를 추가할 것입니다.
...
해당 코드에 대한 설명은 [피킹에 대한 글](picking.html)을 참조하십시오.
...
<iframe src="/editor?url=/manual/examples/threejs-webvr-look-to-select.html"></iframe>
...
먼저 [`OrthographicCamera`](/api/ko/cameras/OrthographicCamera)를 만들어 보겠습니다.
...
다음으로 2가지 색 텍스처를 만들어 봅시다. 몇 군데 [다른](indexed-textures.html) [곳에서](post-processing-3dlut.html) 사용했던 [`DataTexture`](/api/ko/textures/DataTexture)를 사용할 것입니다.
...
그 다음 [`TorusGeometry`](/api/ko/geometries/TorusGeometry)에 있는 텍스처를 사용할 것입니다.
...
[텍스처](textures.html)는 0에서 1까지 정규화된 텍스처 좌표를 사용하여 geometry에 매핑됩니다.
...
[배경과 관련된 글](backgrounds.html)에서 다루었던 것처럼 배경의 질감을 더해봅시다.
...
<iframe src="/editor?url=/manual/examples/threejs-webvr-look-to-select-selector.html"></iframe>

몇 가지 주목하고 <strong>시도해야 할 것들</strong>이 있습니다.
...
이것은 효과의 _역_ 타입으로 주어집니다.
그 세 줄에 주석을 달면 차이를 알 수 있을 것입니다.
저는 역효과가 가장 좋다고 생각하는데, 이렇게 하면 커서의 색깔에 상관없이 커서가 보일 수 있기 때문입니다.

</li>
<li>
[`RingGeometry`](/api/ko/geometries/RingGeometry)가 아닌 [`TorusGeometry`](/api/ko/geometries/TorusGeometry)를 사용해 봅시다.

어떤 이유로든 [`RingGeometry`](/api/ko/geometries/RingGeometry)는 평평한 UV 매핑 방식을 사용합니다.
이 때문에 [`RingGeometry`](/api/ko/geometries/RingGeometry)를 사용하면 위에서처럼 링 주위가 아닌 수평으로 링을 가로질러 텍스처가 미끄러집니다.

이걸 시도해 보고 [`TorusGeometry`](/api/ko/geometries/TorusGeometry)를 [`RingGeometry`](/api/ko/geometries/RingGeometry)(위 예시에서 설명한 대로)로 바꾸면 무슨 뜻인지 알 수 있을 것입니다.

_적절한_ 정의를 위한 _적절한_ 할 것은 [`RingGeometry`](/api/ko/geometries/RingGeometry)를 사용하되 링 주위를 돌도록 텍스처 좌표를 고정하는 것입니다.
아니면, 자신만의 링 지오메트리를 생성하세요. 그래도 torus는 잘 작동합니다.
[`MeshBasicMaterial`](/api/ko/materials/MeshBasicMaterial)과 함께 카메라 바로 앞에 배치하면 링과 똑같이 보이고 텍스처 좌표가 링 주위를 돌기 때문에 우리가 원하는 대로 작동합니다.
...
그리고 세 쌍의 박스와 구 [`Mesh`](/api/ko/objects/Mesh)를 만들어 봅시다. 각 [`Mesh`](/api/ko/objects/Mesh)를 파트너와 연결할 수 있도록 [`맵`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map)을 사용할 것입니다.
...
<iframe src="/editor?url=/manual/examples/threejs-webvr-look-to-select-w-cursor.html"></iframe>

이 예제가 구글 카드 보드 레벨 UX의 "look to select"를 구현하는 방법에 대한 아이디어를 주었기를 바랍니다.
텍스쳐 좌표 오프셋을 사용한 슬라이딩 텍스쳐도 일반적으로 유용한 기법입니다.

다음으로는 [VR 컨트롤러가 있는 사용자가 사물을 가리키고 이동할 수 있는 방법을 알아보겠습니다.](webxr-point-to-select.html).
