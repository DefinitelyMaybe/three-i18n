...
점차 변화하는 것을 말합니다. Three.js에서는 [`Fog`](/api/ko/scenes/Fog)나 [`FogExp2`](/api/ko/scenes/FogExp2) 객체를
생성한 뒤, 장면(scene)의 [`fog`](/api/ko/scenes/Scene#fog) 속성에 지정해 안개를 사용합니다.

[`Fog`](/api/ko/scenes/Fog)는 인자로 `near`와 `far`값을 받는데, 이는 카메라로부터의 거리값입니다.
`near`값보다 가까운 공간은 안개의 영향이 전혀 없고, `far`값보다 먼 공간은
완전히 안개에 뒤덮입니다. `near`와 `far` 사이의 공간에 있는 물체 또는 물체의
일부는 점차 안개의 색으로 변화하죠.

[`FogExp2`](/api/ko/scenes/FogExp2)는 카메라에서 멀어질수록 안개의 강도가 강해집니다.
...
[`FogExp2`](/api/ko/scenes/FogExp2)의 경우는 다음처럼 쓸 수 있죠.
...
[`FogExp2`](/api/ko/scenes/FogExp2)가 더 현실적이긴 하나, 보통 안개의 범위를 특정하기 쉬운
[`Fog`](/api/ko/scenes/Fog)를 더 많이 사용합니다.
...
지정해야 한다는 겁니다. 배경색은 [`scene.background`](/api/ko/scenes/Scene#background)
속성을 [`THREE.Color`](/api/ko/math/Color) 인스턴스로 지정해 바꿀 수 있습니다.
...
안개의 색을 바꾸려면 배경색도 같이 바꿔야 합니다. 헬퍼 클래스에 _가상_ 속성을
...
다행히 [`THREE.Color`](/api/ko/math/Color)에는 [`getHexString`](/api/ko/math/Color#getHexString) 메서드가 있어 색상을
...
추가로 재질(material)에는 불린 타입의 [`fog`](/api/ko/materials/Material#fog) 속성이 있습니다.
