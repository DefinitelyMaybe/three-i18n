...
직관적이라 판단하여 <strong>텍스처</strong>로 번역하였습니다.
...
<ul>
  <li>[하이, 텍스처](/ko/manual/textures#hello)</li>
  <li>[육면체 각 면에 다른 텍스처 지정하기](/ko/manual/textures#six)</li>
  <li>[텍스처 불러오기](/ko/manual/textures#loading)</li>
  <ul>
    <li>[간단한 방법](/ko/manual/textures#easy)</li>
    <li>[텍스처를 불러온 후 처리하기](/ko/manual/textures#wait1)</li>
    <li>[다수의 텍스처를 불러온 후 처리하기](/ko/manual/textures#waitmany)</li>
    <li>[다른 도메인(origin)에서 텍스처 불러오기](/ko/manual/textures#cors)</li>
  </ul>
  <li>[메모리 관리](/ko/manual/textures#memory)</li>
  <li>[JPG vs PNG](/ko/manual/textures#format)</li>
  <li>[필터링과 Mips](/ko/manual/textures#filtering-and-mips)</li>
  <li>
    [텍스처의 반복(repeating), 위치 조절(offseting), 회전(rotating),
    래핑(wrapping)](/ko/manual/textures#uvmanipulation)
  </li>
</ul>

## <a name="hello"></a> 하이, 텍스처

텍스처는 _일반적으로_ 포토샵이나 김프 등의 프로그램으로 만든 이미지입니다.
...
예제는 처음 만들었던 것을 사용하겠습니다. 추가로 [`TextureLoader`](/api/ko/loaders/TextureLoader)를 새로 생성한
뒤, 인스턴스의 [`load`](/api/ko/loaders/TextureLoader#load) 메서드에 이미지의 URL을 넘겨주어 호출하고,
...
※ [`MeshBasicMaterial`](/api/ko/materials/MeshBasicMaterial)을 사용했으므로 광원을 사용할 필요가 없습니다.
...
단순히 재질을 6개 만들어 [`Mesh`](/api/ko/objects/Mesh)를 생성할 때 배열로 넘겨주기만 하면 됩니다.
...
[`BoxGeometry`](/api/ko/geometries/BoxGeometry)나 [`BoxGeometry`](/api/ko/geometries/BoxGeometry)는 최대 6개, [`ConeGeometry`](/api/ko/geometries/ConeGeometry)와
[`ConeGeometry`](/api/ko/geometries/ConeGeometry)는 밑면과 뿔 부분에 하나씩 최대 2개, [`CylinderGeometry`](/api/ko/geometries/CylinderGeometry)와
[`CylinderGeometry`](/api/ko/geometries/CylinderGeometry)는 아래, 위, 옆면 하나씩 최대 3개를 지정할 수 있죠.
...
[`TextureLoader`](/api/ko/loaders/TextureLoader)를 생성하고, 인스턴스의 [`load`](/api/ko/loaders/TextureLoader#load) 메서드를
호출하는 거죠. 이 `load` 메서드는 [`Texture`](/api/ko/textures/Texture) 객체를 반환합니다.
...
다수의 텍스처를 한 번에 불러와야 할 경우 [`LoadingManager`](/api/ko/loaders/managers/LoadingManager)를 사용할 수 있습니다.
[`TextureLoader`](/api/ko/loaders/TextureLoader)를 생성할 때 미리 생성한 [`LoadingManager`](/api/ko/loaders/managers/LoadingManager)의 인스턴스를 인자로
넘겨주고, [`LoadingManager`](/api/ko/loaders/managers/LoadingManager) 인스턴스의 [`onLoad`](/api/ko/loaders/managers/LoadingManager#onLoad) 속성에
콜백 함수를 설정해주는 거죠.
...
[`LoadingManager`](/api/ko/loaders/managers/LoadingManager)의 [`onProgress`](/api/ko/loaders/managers/LoadingManager#onProgress)에 콜백 함수를 지정하면
...
_대체로_ 텍스처는 약 `너비 * 높이 * 4 * 1.33` 바이트의 메모리를 사용합니다.
...
무려 <strong>약 60 메가바이트의 메모리</strong>를 사용합니다. 이런 텍스처가 몇 개만 더
...
넘겨주어야 하는데, GPU는 _일반적으로_ 압축하지 않은 데이터를 사용하죠.
...
텍스처의 크기가 원본보다 클 때의 필터는 [`texture.magFilter`](/api/ko/textures/Texture#magFilter)
...
텍스처가 원본 크기보다 작을 때의 필터는 [`texture.minFilter`](/api/ko/textures/Texture#minFilter)
...
2가지 속성이 있는데, 하나는 수평 래핑을 설정하는 [`wrapS`](/api/ko/textures/Texture#wrapS)이고,
또 하나는 수직 래핑을 설정하는 [`wrapT`](/api/ko/textures/Texture#wrapT)입니다.
...
```js
const xOffset = 0.5; // 텍스처 너비의 반만큼 이동
const yOffset = 0.25; // 텍스처 높이의 1/4만큼 이동
...
텍스처의 `wrapS`나 `wrapT` 속성을 변경할 경우 [`texture.needsUpdate`](/api/ko/textures/Texture#needsUpdate)를
