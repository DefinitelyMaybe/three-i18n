...
렌더링*하는 기법을 말합니다. 중요하니 <strong>한 번 더 읽어보세요!</strong>
...
[`MeshBasicMaterial`](/api/ko/materials/MeshBasicMaterial)을 사용하겠습니다.
...
[`THREE.Object3D`](/api/ko/core/Object3D)를 만들고, 그림자 평면 mesh, 구체 mesh를 이
...
더 자세히 이야기할 거예요. 그림자는 빛을 반사하지 않으니 [`MeshBasicMaterial`](/api/ko/materials/MeshBasicMaterial)을
사용합니다.
...
조명은 2개를 만들겠습니다. 하나는 [`HemisphereLight`](/api/ko/lights/HemisphereLight), 강도를 2로 설정해 화면을 아주
밝게 설정할 겁니다.
...
다른 하나는 구체의 윤곽을 좀 더 분명하게 해 줄 [`DirectionalLight`](/api/ko/lights/DirectionalLight)입니다.
...
[`Math.abs(Math.sin(time))`](</api/ko/math/Math.abs(Math.sin(time))>)를 사용해 구체에 공처럼 통통 튀는
...
이제 그림자 맵을 살펴보겠습니다. 그림자를 드리울 수 있는 조명은 3가지, [`DirectionalLight`](/api/ko/lights/DirectionalLight),
[`PointLight`](/api/ko/lights/PointLight), [`SpotLight`](/api/ko/lights/SpotLight)입니다.

[조명에 관한 글](lights.html)에서 썼던 예제로 먼저 [`DirectionalLight`](/api/ko/lights/DirectionalLight)부터
살펴보죠.
...
[`DirectionalLight`](/api/ko/lights/DirectionalLight)의 위치에 카메라가 있고, 해당 조명의 목표를 바라보는 것이죠. 조명의
그림자에는 별도의 카메라가 있고, 이전에 [카메라에 관한 글](cameras.html)에서
설명한 것처럼 일정 공간 안의 그림자만 렌더링합니다. 위 예제에서는 그 공간이 너무 좁은
것이죠.

그림자용 카메라를 시각화하기 위해 조명의 그림자 속성에서 카메라를 가져와 [`CameraHelper`](/api/ko/helpers/CameraHelper)를
생성한 뒤, 장면에 추가하겠습니다.
...
그림자용 카메라의 속성을 수정하는 GUI를 추가해보죠. [`DirectionalLight`](/api/ko/lights/DirectionalLight)는 빛이 평행으로
나아가므로, [`DirectionalLight`](/api/ko/lights/DirectionalLight)는 그림자용 카메라로 [`OrthographicCamera`](/api/ko/cameras/OrthographicCamera)(정사영 카메라)를
사용합니다. [`OrthographicCamera`](/api/ko/cameras/OrthographicCamera)가 뭔지 잘 기억나지 않는다면, [카메라에 관한 이전 글](cameras.html)을
참고하세요.

[`OrthographicCamera`](/api/ko/cameras/OrthographicCamera)의 시야는 육면체나 *절두체(frustum)*로 정의한다고 했었죠. `left`,
...
[`renderer.capabilities.maxTextureSize`](/api/ko/renderers/WebGLRenderer#capabilities)로 확인할 수 있습니다.

[`SpotLight`](/api/ko/lights/SpotLight)는 그림자용 카메라로 [`PerspectiveCamera`](/api/ko/cameras/PerspectiveCamera)(원근 카메라)를 사용합니다. [`DirectionalLight`](/api/ko/lights/DirectionalLight)의
그림자용 카메라는 거의 모든 속성을 직접 변경할 수 있었지만, [`SpotLight`](/api/ko/lights/SpotLight)의 그림자용 카메라는
조명 속성의 영향을 받습니다. 카메라의 `fov` 속성은 [`SpotLight`](/api/ko/lights/SpotLight)의 `angle` 속성과 직접 연결되어
...
마지막으로 [`PointLight`](/api/ko/lights/PointLight)를 살펴보죠. [`PointLight`](/api/ko/lights/PointLight)는 모든 방향으로 빛을 발산하기에
관련 설정은 `near`와 `far` 정도입니다. 그리고 사실 [`PointLight`](/api/ko/lights/PointLight)의 그림자는 정육면체의
각 면에 [`SpotLight`](/api/ko/lights/SpotLight)를 하나씩, 총 6개의 그림자를 놓은 것과 같습니다. 한 방향에 한
...
물론 조명도 [`PointLight`](/api/ko/lights/PointLight)로 바꿔야죠.
