...
다음으로 [`MeshPhongMaterial`](/api/ko/materials/MeshPhongMaterial)의 `emissive(방사성)` 속성(property)을
...
지구는 푸른색을 사용했으나, 약간의 _방사성(emissive)_ 파랑을 섞어
...
여기서는 [`Object3D`](/api/ko/core/Object3D)를 생성했습니다. [`Object3D`](/api/ko/core/Object3D)는 [`Mesh`](/api/ko/objects/Mesh)와 마찬가지로
...
이전처럼 [`Object3D`](/api/ko/core/Object3D)를 이용해 `eathOrbit` "지역 공간"을 만들고 거기에
...
그 중 하나는 [`AxesHelper`](/api/ko/helpers/AxesHelper)로, 이 클래스는 지역

<span style="color:red">X</span>,<span style="color:green">Y</span>,
<span style="color:blue">Z</span> 축을 표시해줍니다. 한 번 여태까지 만든 요소에
모두 추가해보죠.
...
아래를 내려다 보고, 각 물체도 y축을 따라 회전하므로
<span style="color:green">y축(초록)</span>은 보여도 거의 점처럼 보일 겁니다.
...
[`GridHelper`](/api/ko/helpers/GridHelper)도 추가해보겠습니다. [`GridHelper`](/api/ko/helpers/GridHelper)는 X, Z축으로 2D 격자(grid)를
...
각 요소에 [`GridHelper`](/api/ko/helpers/GridHelper)와 [`AxesHelper`](/api/ko/helpers/AxesHelper)를 추가하겠습니다. 각 노드에 헬퍼를
...
속성을 바꿀 때 요소의 [`AxesHelper`](/api/ko/helpers/AxesHelper)와 [`GridHelper`](/api/ko/helpers/GridHelper)의 속성을
동시에 조작할 수 있죠.
...
격자가 축을 가릴 수 있으니, [`AxesHelper`](/api/ko/helpers/AxesHelper)의 `renderOrder`를
2로 설정하고 [`GridHelper`](/api/ko/helpers/GridHelper)를 2로 설정해 축을 격자 다음에
렌더링하도록 합니다.
...
[`Object3D`](/api/ko/core/Object3D)는 청색, 광원은 갈색, 카메라는 보라색으로 칠했습니다. 하나의
...
`targetOrbit`([`Object3D`](/api/ko/core/Object3D))을 만듭니다. 그리고 `targetOrbit`의 상대 좌표를 넘겨줄
`targetElevation`([`Object3D`](/api/ko/core/Object3D))을 만들어 `targetOrbit`의 자식으로 추가한 뒤,
또 다른 [`Object3D`](/api/ko/core/Object3D), `targetBob`을 만들어 `targetElevation`의 자식으로 추가합니다.
...
탱크는 먼저 `tank`라는 이름으로 다른 요소를 감쌀 [`Object3D`](/api/ko/core/Object3D)를 하나 생성합니다.
예제에서는 커브에 따라 위치값을 반환받을 수 있는 [`SplineCurve`](/api/ko/extras/curves/SplineCurve)를 이용하겠습니다.
0.0은 커브의 시작점이고, 1.0은 커브의 끝점으로, 먼저 탱크의 위치를 넘겨주어 탱크의
다음 위치를 정한 뒤(아래 `tankPosition`. 역주), 커브의 다음 값을 받아 탱크가 어디를
바라봐야할지 구합니다(아래 `tankTarget`. 역주). 그리고 구한 값을 [`Object3D.lookAt`](/api/ko/core/Object3D.lookAt)
...
그리고 목표물의 전역 위치값(global position)을 구한 뒤 [`Object3D.lookAt`](/api/ko/core/Object3D.lookAt)
...
어떻게 사용해야할지 감을 잡는 데 도움이 되었으면 좋겠네요. [`Object3D`](/api/ko/core/Object3D)
