...
모듈이 바뀌었으니 import 문도 변경해야 합니다. [`OBJLoader`](/examples/loaders/OBJLoader)를 제거하고 [`GLTFLoader`](/examples/loaders/GLTFLoader)를
추가합니다.

```html
-import { LoaderSupport } from 'three/addons/loaders/LoaderSupport.js'; -import
{ OBJLoader } from 'three/addons/loaders/OBJLoader.js'; -import { MTLLoader }
from 'three/addons/loaders/MTLLoader.js'; +import { GLTFLoader } from
...
각각의 자동차에 별도의 [`Object3D`](/api/ko/core/Object3D)를 만들어 자동차를 이 [`Object3D`](/api/ko/core/Object3D)의 자식으로
지정할 겁니다. 이러면 자동차가 아닌 [`Object3D`](/api/ko/core/Object3D)로 차를 움직일 수 있고, 자동차의
기준축도 별도로 설정할 수 있죠.

아까 봤던 씬 그래프를 다시 보니 자동차의 종류는 총 3개인 듯합니다. "Car_08",
"CAR_03", "Car_04" 이렇게요. 종류별로 조정했을 때 제대로 적용되는지 봅시다.

아래의 코드는 각 자동차를 새로운 [`Object3D`](/api/ko/core/Object3D)의 자식으로 지정하고, 이 [`Object3D`](/api/ko/core/Object3D)를
장면에 추가한 뒤, 자동차의 _종류별로_ 기준축을 정렬합니다. 그리고 새로 만든
[`Object3D`](/api/ko/core/Object3D)를 `cars` 배열에 추가하죠.
...
Three.js에는 몇 가지 곡선 클래스가 있습니다. 이 경우 [`CatmullRomCurve3`](/api/ko/extras/curves/CatmullRomCurve3)가
...
지점에 하나를 새로 만들어 [`CatmullRomCurve3`](/api/ko/extras/curves/CatmullRomCurve3)에 넘겨주겠습니다.
...
[그림자에 관한 글](shadows.html)의 [`DirectionalLight`](/api/ko/lights/DirectionalLight) 그림자 예제를 가져와
그대로 코드에 붙여 넣습니다.
...
마찬가지로 <strong>존재하는 모든 기능을 지원하지는 못합니다</strong>.
