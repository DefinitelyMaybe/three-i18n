...
[이전 글](canvas-textures.html)에서는 [`CanvasTexture`](/api/ko/textures/CanvasTexture)를 이용해 캐릭터에 이름표/명찰을 붙이는 법을 알아봤습니다. 때로는 이 이름표가 항상 카메라를 향하게 해야 할 경우도 있겠죠. Three.js의 [`Sprite`](/api/ko/objects/Sprite)와 [`SpriteMaterial`](/api/ko/materials/SpriteMaterial)을 사용하면 이걸 쉽게 구현할 수 있습니다.

[캔버스 텍스처에 관한 글](canvas-textures.html)의 명찰 예제를 가져와 여기에 [`Sprite`](/api/ko/objects/Sprite)와 [`SpriteMaterial`](/api/ko/materials/SpriteMaterial)을 사용해봅시다.
...
다음으로 함수를 하나 만듭니다. 이 함수는 나무 줄기 [`Mesh`](/api/ko/objects/Mesh)와 윗부분 [`Mesh`](/api/ko/objects/Mesh)를 만들고 이 둘을 [`Object3D`](/api/ko/core/Object3D)의 자식으로 넣는 역할입니다.
