...
각각 캔버스를 만들고, 각 캔버스마다 [`Renderer`](/api/ko/constants/Renderer)를 생성하면 되지 않을까요?

하지만 이 방법을 적용하자마자 문제가 생깁니다.

<ol>
<li>
브라우저의 WebGL 컨텍스트(context)는 제한적이다.
...
두는 겁니다. [`Renderer`](/api/ko/constants/Renderer)는 하나만 만들되 가상 캔버스에 각각 [`Scene`](/api/ko/scenes/Scene)을 만드는 거죠. 그리고
...
이제 각 요소가 화면에 보일 때만 장면을 렌더링할 함수를 만듭니다. [`Renderer.setScissorTest`](/api/ko/constants/Renderer.setScissorTest)를
호출해 _가위(scissor)_ 테스트를 활성화하면 Three.js가 캔버스의 특정 부분만 렌더링하도록
할 수 있습니다. 그리고 [`Renderer.setScissor`](/api/ko/constants/Renderer.setScissor)로 가위를 설정한 뒤 [`Renderer.setViewport`](/api/ko/constants/Renderer.setViewport)로
장면의 좌표를 설정합니다.
...
그런 다음 [빠르게 스크롤을 위아래로 반복해보면](../examples/multiple-scenes-v2.html)
...
HTML의 [dataset](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/dataset)을
