...
정육면체를 투명하게 만들려면 [`transparent`](/api/ko/materials/Material#transparent) 속성을
켜고 [`opacity`](/api/ko/materials/Material#opacity) 속성을 설정해줘야 합니다(CSS와 마찬가지로
...
이전에 [재질(material)에 관해](materials.html) 배울 때 [`side`](/api/ko/materials/Material#side)
...
먼저 렌더링해야 합니다. [`Mesh`](/api/ko/objects/Mesh) 같은 경우는 Three.js가 자동으로 이를 처리해주죠.
...
[`PlaneGeometry`](/api/ko/geometries/PlaneGeometry)를 썼습니다. [이전에 다뤘듯](textures.html)
이 geometry는 기본적으로 텍스처를 크기에 맞춰 늘립니다. [`texture.repeat`](/api/ko/textures/Texture#repeat)
속성과 [`texture.offset`](/api/ko/textures/Texture#offset) 속성을 조정해 각 면에 적절한
텍스처를 입혀줄 수 있죠.

위 코드에서는 [`Object3D`](/api/ko/core/Object3D)를 만들어 두 평면의 부모로 지정했습니다. 이렇게
하면 복잡한 계산 없이 간단하게 [`Object3D`](/api/ko/core/Object3D)만 돌려서 두 평면 다 회전시킬
수 있죠.
...
알파 테스트란 Three.js가 픽셀을 렌더링하지 않는 특정 _알파_ 단계를 의미합니다.
...
[`alphaTest`](/api/ko/materials/Material#alphaTest) 속성을 지정하겠습니다.
