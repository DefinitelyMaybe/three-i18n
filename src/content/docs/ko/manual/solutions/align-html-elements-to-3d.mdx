...
[원시 모델에 관한 글](primitives.html)을 보면 [`TextGeometry`](/api/ko/geometries/TextGeometry)로 3D 텍스트를 만든 예제를 찾을 수 있을 겁니다. 로고에 애니메이션을 준다던가 하는 경우에는 유용하지만 상태, 정보, 이름 등을 붙이는 경우라면 오히려 불편하겠죠.
...
여기에 [`OrbitControls`](/examples/controls/OrbitControls)를 넣습니다. [조명에 관한 글](lights.html)에서 다뤘었죠.
