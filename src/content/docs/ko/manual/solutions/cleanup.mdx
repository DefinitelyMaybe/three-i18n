...
불러온 파일을 해제하려면 코드를 좀 더 추가해야 합니다. Three.js의 로더는 대부분 최상위 [`Object3D`](/api/ko/core/Object3D)만을 반환하기에 어떤 자원을 사용했는지 체크하려면 일일이 하위 요소를 뒤져봐야 합니다.

`ResourceTracker`를 업데이트해 저 역할을 맡겨보죠.

먼저 자원이 [`Object3D`](/api/ko/core/Object3D)인지 확인해 해당 요소의 geometry, 재질, 하위 요소를 추적하도록 합니다.
