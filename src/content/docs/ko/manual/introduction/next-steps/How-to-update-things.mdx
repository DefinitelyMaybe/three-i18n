...
BufferGeometries는 [buffers](api/core/BufferAttribute)에 다양한 정보(꼭짓점 위치, 면 순서, 법선, 색깔,
UV, 모든 커스텀 속성들)를 저장합니다. - 이는
[ typed arrays](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Typed_arrays)라는 의미입니다.
...
렌더링 시점에 확장되는 선을 예로 들어보겠습니다. buffer에는 500 개의 꼭짓점을 할당하겠지만 처음에는 [BufferGeometry](api/core/BufferGeometry#drawRange)를 이용해 두 개만 그릴 것입니다..
...
첫 렌더링 이후에 *점의 갯수*를 변경하고 싶다면, 다음과 같이 실행하세요:
...
[ 다음 코드](https://jsfiddle.net/t4m85pLr/1/)는 사용 사례에 맞게 조정할 수 있는 애니메이션 라인을 보여줍니다.
