...
[skinned and rigged model](api/objects/SkinnedMesh)의 뼈, 모프타깃, 서로 다른 재질의 속성(색상, 불투명도, 참/거짓 연산),
...
이를 한 시스템 안에 구현하기 위해서, three.js 애니메이션 시스템은 2015년에 완전히 변경되었으며([ Link](https://github.com/mrdoob/three.js/issues/6881))
...
[ glTF Blender exporter](https://github.com/KhronosGroup/glTF-Blender-IO)라는 익스포터로 추출하고
[GLTFLoader](examples/loaders/GLTFLoader)를 사용해 three.js에 불러왔다면 — 응답 필드 중 하나가 "animations"라는 이름의 배열로 되어 있고 해당 모델에 대한
[AnimationClips](api/animation/AnimationClip)를 담고 있을 것입니다(활용 가능한 로더는 아래 리스트를 확인하세요).
...
이러한 _AnimationClip_ 안에는 각각의 애니메이션 속성별 데이터가 별도의 [KeyframeTrack](api/animation/KeyframeTrack)에 저장되어 있습니다.
[skeleton](api/objects/Skeleton) 이라는 오브젝트가 있다고 가정하면, 한 키프레임 트랙은 하완골격의 위치가 수시로 변화하는 데이터를 저장할 수 있을 것이며
...
해당 모프 타깃이 변화하는 동안에 어떻게 [영향](api/objects/Mesh#morphTargetInfluences)을 미치는지에 대한 정보를 담고 있을 것입니다.

### 애니메이션 믹서(Animation Mixer)

저장된 데이터 폼은 오직 애니메이션에 대한 기본 정보일 뿐입니다. 실제 플레이백은 [AnimationMixer](api/animation/AnimationMixer)가 담당합니다.
...
[AnimationActions](api/animation/AnimationAction)으로 설정할 수 있습니다. *AnimationAction*을 설정하면
...
[AnimationObjectGroup](api/animation/AnimationObjectGroup)을 사용하면 됩니다.

### 지원되는 포맷 및 로더

애니메이션을 포함한 모든 모델들(OBJ는 확실히 안 됩니다)이 지원되는 것은 아니며, 아주 몇몇
three.js 로더만 [AnimationClip](api/animation/AnimationClip) 시퀀스를 지원합니다. 지원*되는*
애니메이션 타입은 다음과 같습니다:
