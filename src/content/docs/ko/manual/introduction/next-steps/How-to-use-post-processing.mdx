...
three.js는 완벽한 후처리 솔루션인 [EffectComposer](examples/postprocessing/EffectComposer)를 통해 작업 환경을 구현합니다.

## 작업 절차

먼저 해야 할 일은 examples 디렉토리의 모든 필요한 파일들을 불러오는 것입니다. three.js의 공식 가이드
[ npm package](https://www.npmjs.com/package/three)를 따르고 있다고 가정합니다.
...
모든 파일들을 잘 불러왓다면, composer를 만들어 [WebGLRenderer](api/renderers/WebGLRenderer)인스턴스를 넘겨줍니다.

```javascript
const composer = new EffectComposer(renderer);
```

composer를 사용할 때, 앱의 애니메이션 루프를 변경해주는 것이 중요합니다. [WebGLRenderer](api/renderers/WebGLRenderer)의 render 메서드를 호출하지 말고,
이제부터는 각각의 [EffectComposer](examples/postprocessing/EffectComposer)에 대응되는 렌더링 방법을 사용합니다..
...
[ live example](https://threejs.org/examples/webgl_postprocessing_glitch)에서 작동을 확인해보세요.

## 기본 내장 후처리방식

엔진에서 제공하는 여러 내장 후처리 방식을 활용할 수 있습니다. 이 방식들은
[ postprocessing](https://github.com/mrdoob/three.js/tree/dev/examples/jsm/postprocessing) 디렉토리에 있습니다.
...
[ CopyShader](https://github.com/mrdoob/three.js/blob/master/examples/jsm/shaders/CopyShader.js)에서 처음 자신만의 셰이더를 만들기에 적합한 파일을 제공하고 있습니다.
*CopyShader*는 [EffectComposer](examples/postprocessing/EffectComposer)의 쓰기 리드 버퍼에 아무 효과를 적용하지 않은 채 이미지 컨텐츠를 복사하기만 합니다.
